...
function renderRequestModal() {
  ...
  modal.querySelectorAll(".req-card").forEach(card => {
    card.addEventListener("click", () => {
      ...
      switch (type) {
        case "Travel Order":
          alert("Travel Order form not yet implemented");
          // mountTravelOrderForm();
          break;

        case "Driver Trip Ticket":
          alert("Driver Trip Ticket form not yet implemented");
          // mountDriverTripForm();
          break;
...
function attachRequestModal() {
  ...
  modal.querySelectorAll(".req-card").forEach(card => {
    card.addEventListener("click", () => {
      const type = card.dataset.type;
      alert(`Selected: ${type}`); // later replace with opening actual form
      modal.remove();
    });
  });
}
