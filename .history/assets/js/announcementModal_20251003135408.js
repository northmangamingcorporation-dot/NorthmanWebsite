...
(function() {
  ...
  style.textContent = `
    ...
    /* Hide default scrollbars but keep scroll functionality */
...
// Add this new function to store dismissed announcements
async function markAnnouncementAsViewed(userId, announcementId, collectionName) {
  try {
    await window.db.collection('viewed_announcements').add({
      userId: userId,
...
async function checkAnnouncementDatabases(user) {
 ...
 await Promise.all(promises);
  ...
  try {
    const viewedSnapshot = await window.db.collection('viewed_announcements')
      .where('userId', '==', user.username || user.email)
      .where('dismissed', '==', true)
      .get();
    
    const viewedIds = new Set();
    viewedSnapshot.forEach(function(doc) {
      const data = doc.data();
      viewedIds.add(data.collectionName + '_' + data.announcementId);
    });
    
    // Filter out viewed announcements
    const filteredAnnouncements = announcements.filter(function(ann) {
      return !viewedIds.has(ann.collectionName + '_' + ann.id);
    });
    
    announcements.length = 0;
    filteredAnnouncements.forEach(function(ann) {
      announcements.push(ann);
    });
  } catch (error) {
    console.error('Error fetching viewed announcements:', error);
  }
