...
class LogoutManager {
  ...
  redirectToHome() {
    // Try to use the global mount function if available
    if (window.renderHome && window.mount) {
      try {
        window.mount(window.renderHome());
        return;
      } catch (error) {
        console.warn('Error mounting home page, using fallback:', error);
      }
    }

    // Fallback 1: Try to reload the page
    try {
      window.location.reload();
      return;
    } catch (error) {
      console.warn('Error reloading page, using fallback:', error);
    }

    // Fallback 2: Direct redirect to index.html
    try {
      ...
      alert('Logout successful. Please refresh the page to return to home.');
    }
  ...
  async logoutWithRedirect(redirectTo) {
    return this.logout({ showConfirmation: true, redirectTo });
  }
}
...
// Auto-initialize logout manager
