...
function injectMainPageStyles() {
  ...
  style.textContent = `
    ...
    * {
    ...
    /* Values Section - Card variants and core values grid (FIXED: Completed formatting) */
    .ngc-values-section {
      background: var(--ngc-surface);
      max-width: 1200px;
      margin: 0 auto;
    ...
    /* Enhanced Footer - Dark theme with improved grid (FIXED: Added fallback support) */
    .ngc-footer {
      background: var(--ngc-text-primary);
      color: white;
    ...
    .ngc-footer-logo .ngc-logo-fallback {
      flex: 1; /* Fill container if img fails */
    }
  ...
  console.log("NGC styles injected successfully (with logo fallbacks).");
}
...
// Enhanced preloader for images with progress tracking and retry (FIXED: Use LOGO_PATH, add retry)
function preloadNGCImages() {
  const imageUrls = [LOGO_PATH]; // Use centralized path
  ...
  imageUrls.forEach((url, index) => {
    ...
    img.onload = () => {
      loadedCount++;
      console.log(`Image ${index + 1}/${totalImages} loaded successfully: ${url}`);
      if (loadedCount === totalImages) {
        console.log("All NGC images preloaded successfully.");
      }
    };
    img.onerror = (e) => {
      console.error(`Failed to preload image: ${url}. Error:`, e);
      console.warn(`Suggestion: Verify file exists at ${url}. If missing, add it or update LOGO_PATH.`);
      // Retry once with a placeholder URL
      setTimeout(() => {
        img.src = 'https://via.placeholder.com/48x48/3b82f6/ffffff?text=NGC'; // Fallback placeholder
        console.log(`Retrying with placeholder for: ${url}`);
      }, 1000);
    };
    img.src = url;
  });
...
// Enhanced initial mount with better error handling, session validation, and performance metrics (FIXED: localStorage key without space)
document.addEventListener("DOMContentLoaded", () => {
  try {
    ...
    const loggedInUser  = localStorage.getItem("loggedInUser "); // FIXED: No trailing space

    if (loggedInUser ) {
      try {
        const user = JSON.parse(loggedInUser ); // FIXED: No space
        ...
        if (user.expiresAt && Date.now() > user.expiresAt) {
          localStorage.removeItem("loggedInUser "); // FIXED: No space
...
// Global resize handler (debounced) - FIXED: Consistent usage
...
// Store observer globally for cleanup (updated in initializeScrollAnimations)
let ngcObserver;
