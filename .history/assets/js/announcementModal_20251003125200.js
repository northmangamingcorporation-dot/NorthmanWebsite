  ...
  return `
    <div class="announcement-loading">
      <div class="loading-content">
        ...
        <span class="loading-text">Checking for updates...</span>
      </div>
    </div>
  `;
}
...
function renderAnnouncement(announcement, currentIndex, totalCount, department) {
  ...
  const quickActionsHtml = announcement.allowQuickActions && 
    (announcement.status === 'pending' || announcement.status === 'for_approval') ? `
    <div class="announcement-quick-actions">
      <button class="quick-action-btn btn-view-details" data-action="view" data-id="${announcement.id}">
        <i class="fas fa-eye"></i>
        <span>View</span>
      </button>
      <button class="quick-action-btn btn-deny" data-action="deny" data-id="${announcement.id}">
        <i class="fas fa-times"></i>
        <span>Deny</span>
      </button>
      <button class="quick-action-btn btn-approve" data-action="approve" data-id="${announcement.id}">
        <i class="fas fa-check"></i>
        <span>Approve</span>
      </button>
    </div>
  ` : announcement.allowQuickActions ? `
    <div class="announcement-quick-actions">
      <button class="quick-action-btn btn-view-details" data-action="view" data-id="${announcement.id}">
        <i class="fas fa-eye"></i>
      ...
      quickActionBtns.forEach(function(btn) {
        btn.addEventListener('click', async function() {
          ...
          if (action === 'view') {
            ...
            // Trigger view details event with announcement data
            const event = new CustomEvent('viewAnnouncementDetails', {
              detail: {
                announcement: currentAnnouncement,
                collection: currentAnnouncement.collectionName,
                documentId: currentAnnouncement.id,
                data: currentAnnouncement.rawData
              }
            });
            window.dispatchEvent(event);
            
            // Also call global handler if exists
            if (window.handleViewAnnouncementDetails) {
              window.handleViewAnnouncementDetails(currentAnnouncement);
            }
          ...
          if (action === 'approve') {
            success = await handleQuickApprove(currentAnnouncement);
          } else if (action === 'deny') {
            success = await handleQuickDeny(currentAnnouncement);
          }
...
// Listen for view details event (example handler)
window.addEventListener('viewAnnouncementDetails', function(e) {
  console.log('View details triggered for:', e.detail);
  console.log('Collection:', e.detail.collection);
  console.log('Document ID:', e.detail.documentId);
  console.log('Full data:', e.detail.data);
  
  // Example: You can implement your custom logic here
  // Navigate to specific page, show detailed modal, etc.
});

// Global handler example - You can override this in your main application
window.handleViewAnnouncementDetails = function(announcement) {
  console.log('Viewing details for:', announcement.title);
  console.log('Collection:', announcement.collectionName);
  console.log('ID:', announcement.id);
  
  // Example implementations:
  
  // Option 1: Navigate to specific page
  // window.location.href = `/view/${announcement.collectionName}/${announcement.id}`;
  
  // Option 2: Show in a modal
  // if (window.Modal && window.Modal.showDetails) {
  //   window.Modal.showDetails(announcement);
  // }
  
  // Option 3: Open in sidebar
  if (window.Sidebar && window.Sidebar.showDetails) {
    window.Sidebar.showDetails(announcement.collectionName, announcement.id);
  }
  
  // For now, just alert (replace with your actual implementation)
