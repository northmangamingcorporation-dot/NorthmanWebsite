...
async function handleQuickApprove(announcement) {
  try {
    const updates = {
      status: 'approved',
      approvedAt: new Date(),
      approvedBy: announcement.currentUserId || 'system'
    };
...
async function handleQuickDeny(announcement) {
  try {
    const updates = {
      status: 'denied',
      deniedAt: new Date(),
      deniedBy: announcement.currentUserId || 'system'
    };
...
window.addEventListener('viewAnnouncementDetails', function(e) {
  ...
  // Example: Navigate to specific page or show detailed modal
});

console.log('Advanced Department-Based Announcement Modal System loaded.');
console.log('Usage: showAnnouncementModal({ id: "userId", department: "IT" })');