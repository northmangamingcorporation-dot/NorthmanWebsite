...
function attachDriversTripTicketForm(preFillUsername = "") {
  ...
  const previewDiv = form.querySelector(`.${inputName.includes("departure") ? "departurePreview" : "arrivalPreview"}`);
    ...
    input.addEventListener("change", () => {
        ...
        const confirmPreview = document.getElementById("confirmPreviewImage");
        confirmPreview.src = e.target.result; // Set preview image
        ...
        confirmBtn.onclick = async () => {
            ...
            const uploaded = await window.GoogleDrive.uploadFile(file);
            if (uploaded) {
            alert(`✅ File uploaded: ${uploaded.name}`);
            form.querySelector(`input[name="${inputName}Url"]`)?.value = uploaded.webViewLink;
            } else {
            alert("❌ Upload failed");
            }
        };
