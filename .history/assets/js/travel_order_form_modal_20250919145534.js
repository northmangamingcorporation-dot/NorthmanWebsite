...
function attachTravelOrderForm(preFillUsername = "") {
  ...
  try {
    ...
    const telegramData = { ...data }; // copy data
    // Convert passengers array to string for Telegram
    telegramData.passengers = passengers.join(", ");

    // Build message text
    const captionText = Object.entries(telegramData)
      .map(([k, v]) => `${k}: ${v}`)
      .join("\n");

    // Send to Telegram
    const BOT_TOKEN = "8371293203:AAEGWdqThHuXSWDf6CKDa3e74hgbqTxDYP4"; // Testing only
