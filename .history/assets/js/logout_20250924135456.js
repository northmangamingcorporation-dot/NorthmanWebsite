...
class LogoutManager {
  ...
  cleanupDOMHandlers() {
    try {
      ...
      // Disable all buttons to prevent clicks after logout
      ...
      elementsToClean.forEach(selector => {
        ...
        if (element && element.removeEventListener) {
          // Common events that might cause issues
          ['click', 'scroll', 'resize', 'beforeunload'].forEach(eventType => {
            try {
      ...
      console.log('DOM cleanup completed, modals hidden, home buttons preserved');
    } catch (error) {
