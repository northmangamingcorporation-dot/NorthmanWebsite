...
function attachDriversTripTicketForm(preFillUsername = "") {
  ...
  // Handle file inputs & preview (no confirmation modal)
  ...
  // Collect form values EXCLUDING file inputs
  for (let [key, value] of formData.entries()) {
    if (key !== "departureProof" && key !== "arrivalProof") {
      data[key] = value;
    }
  }

  try {
    // Save only non-file data to Firestore
    ...
    const filesToSend = [];
    ["departureProof", "arrivalProof"].forEach((inputName) => {
      const file = form.querySelector(`input[name="${inputName}"]`)?.files[0];
      if (file) filesToSend.push({ key: inputName, file });
    });

    // Send ticket to Telegram with files
    ...
    document.querySelectorAll('.departurePreview, .arrivalPreview').forEach(div => {
      div.innerHTML = '<span style="font-size:12px; color:#888;">No Image</span>';
    });
  } catch (err) {
  ...
  // Fullscreen image preview & zoom/pan
