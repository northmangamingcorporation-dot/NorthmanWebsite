 ...
 */
/**
 * Parse API response and extract stats for the next upcoming draw
 * Returns numbers only
 */
function parseApiResponse(response) {
  const stats = {
    pending: 0, // just a number now
    approved: 0,
    denied: 0,
    payout: 0
  };
  ...
  // Extract next draw pending count
  let pendingCount = 0;
  ...
  stats.pending = pendingCount; // now just a number
  return stats;
}
