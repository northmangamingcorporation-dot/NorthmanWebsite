 ...
 * ✅ Fixed: Handle the actual API response structure with per-draw pending
 */
function parseApiResponse(response) {
  ...
  if (response.latest_update?.combined_report) {
    ...
    if (report.comprehensive_stats?.cancellations?.daily) {
      // Get per-draw pending counts
      const byDraw = report.comprehensive_stats.cancellations.by_draw || {};
      const pendingByDraw = {};
      
      Object.entries(byDraw).forEach(([drawTime, counts]) => {
        if (counts.pending > 0) {
          pendingByDraw[drawTime] = counts.pending;
        }
      });
      
      stats.pending = pendingByDraw; // Pass as object for per-draw display
