...
async function mountAdminTravelOrderModal(admin, orderData) {
  ...
  driverDropdown.innerHTML = `<option value="">-- Assign Driver --</option>`;
  const staffSnap = await db.collection("clients").where("position", "==", "Driver").get();
  staffSnap.forEach(doc => {
    const u = doc.data();
    driverDropdown.innerHTML += `<option value="${u.username}">${u.username}</option>`;
  });
