  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (file) {
        ...
        // No file - save without proof (though validation requires it, this is fallback)
        if (!window.db) {
          throw new Error("Database not initialized.");
        }
        await window.db.collection("drivers_trip_tickets").doc(data.id).set(data);
        ...
        if (window.Modal && window.Modal.show) {
          window.Modal.show(`Driver's Trip Ticket submitted successfully!\nReference ID: ${data.id}\nNote: No proof photo attached.`, "success");
        } else {
          alert(`✅ Driver's Trip Ticket Submitted!\nReference ID: ${data.id}\nNote: No proof photo attached.`);
        }
        ...
        // Optional: Send via Telegram if available
        if (window.TelegramConnect && typeof window.TelegramConnect.sendRequest === "function") {
          window.TelegramConnect.sendRequest(data, "Driver's Trip Ticket");
        }
      ...
      console.error("Error submitting Driver's Trip Ticket:", error);
      loadingOverlay.style.display = "none";
      submitBtn.disabled = false;
      submitBtn.innerHTML = '<i class="fas fa-paper-plane"></i> Submit Departure';

      let errorMessage = "Failed to submit Driver's Trip Ticket. Please try again.";
      if (error.code === 'permission-denied') {
        errorMessage = "Permission denied. Please check your account.";
      } else if (error.message.includes('network')) {
        errorMessage = "Network error. Please check your connection.";
      } else if (error.message.includes('5MB')) {
        errorMessage = "File too large. Please upload an image under 5MB.";
      }
...
function mountDriversTripTicketForm(user) {
  try {
    if (!user || !user.username) {
      console.error("No valid user provided for mountDriversTripTicketForm.");
      if (window.Modal && window.Modal.show) {
        window.Modal.show("Please log in to submit a driver's trip ticket.", "warning");
      }
      return;
    }
    ...
    // Remove existing trip ticket modal if present
    const existingModal = document.getElementById("driversTripTicketModal");
    if (existingModal) {
      existingModal.remove();
    }

    // Render and insert
    const html = renderDriversTripTicketForm(user);
    document.body.insertAdjacentHTML("beforeend", html);

    // Attach functionality
    attachDriversTripTicketForm(user);
    ...
    setTimeout(() => {
      const firstField = document.querySelector('#driversTripTicketForm input[required]');
      if (firstField) firstField.focus();
    }, 100);

    console.log("Driver's Trip Ticket form mounted successfully.");

  } catch (error) {
    console.error("Error mounting Driver's Trip Ticket form:", error);
    if (window.Modal && window.Modal.show) {
      window.Modal.show("Unable to load Driver's Trip Ticket form. Please try again.", "error");
    } else {
      alert("Unable to load Driver's Trip Ticket form. Please refresh the page.");
    }
  }
}
...
window.openImagePreview = window.openImagePreview || (() => {});  // Ensure global access to preview function
...
if (!document.querySelector('style[data-drivers-fadeout]')) {
  const style = document.createElement("style");
  style.setAttribute('data-drivers-fadeout', 'true');
  style.textContent = `
    @keyframes fadeOut {
      from { opacity: 1; }
      to { opacity: 0; }
...
window.addEventListener('error', (event) => {
  if (event.filename && event.filename.includes('drivers_trip_ticket_form_modal.js')) {
    console.error('Driver\'s Trip Ticket Form Error:', {
