...
function attachTravelOrderForm(user) {
  ...
  waitForElements(({ modal, form, loadingOverlay, submitBtn, addPassengerBtn, passengersWrapper, closeBtn, cancelBtn, passengersError }) => {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      submitBtn.disabled = true;
      form.querySelectorAll('input, textarea, button:not(.remove-passenger)').forEach(el => el.disabled = true);
      ...
      try {
        ...
        // Collect passengers (only non-empty, uppercased)
        data.passengers = [];
        ...
        console.log("Full data prepared:", data);
        ...
        passengerCount = 1;
        // Re-attach listeners to new initial input
        const newInitialInput = passengersWrapper.querySelector('input[name="passenger[]"]');
        if (newInitialInput) {
          newInitialInput.addEventListener('input', debounce(validateForm, 300));
          newInitialInput.addEventListener('change', validateForm);
        }
        validateForm(); // Re-run to disable button post-reset

      } catch (error) {
        console.error("Error submitting Travel Order:", error);
        loadingOverlay.style.display = "none";
        submitBtn.disabled = false;
        form.querySelectorAll('input, textarea, button:not(.remove-passenger)').forEach(el => el.disabled = false);
        submitBtn.innerHTML = submitBtn.classList.contains('valid') ? '<i class="fas fa-check"></i> Submit Request' : '<i class="fas fa-times" style="color: #ef4444;"></i> Complete Form First';
        submitBtn.style.cursor = submitBtn.classList.contains('valid') ? 'pointer' : 'not-allowed';
        ...
        validateForm(); // Re-validate after error
      }
    });
...
// Enhanced mount function (with initial validation)
function mountTravelOrderForm(user) {
  try {
    ...
    setTimeout(() => {
      const firstField = document.querySelector('#travelOrderForm input[required]:first-of-type');
