...
// Initialize ChartJS logic for one teller (1-week report)
async function initializeErrorChartModal(tellerName) {
  ...
  // Fetch Firestore data (filter by submittedAt >= weekAgo)
  ...
  // Initialize all days with 0 (for a full 7-day view even if no errors)
  for (let i = 0; i < 7; i++) {
  ...
  // Count errors only for the selected teller
  snapshot.forEach(doc => {
    const data = doc.data();
    ...
    const dateKey = new Date(submittedAt).toLocaleDateString("en-US", { month: "short", day: "numeric" });
  ...
  new window.Chart(ctx, {
    ...
    data: {
      labels,
      datasets: [{
        label: `Errors per Day (${tellerName}) - Last 7 Days`,
        data: values,
