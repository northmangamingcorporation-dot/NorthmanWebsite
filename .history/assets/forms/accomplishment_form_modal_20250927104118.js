...
function attachAccomplishmentForm(user) {
  ...
  // UPDATED: Form submission - Save details to Firestore, send photos to Telegram
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // NO Firebase Storage upload - photos go directly to Telegram
      ...
      // Send details + photos to Telegram (non-blocking)
      if (window.TelegramConnect && typeof window.TelegramConnect.sendDocument === "function") {
        try {
          // Pass data (details), type, and selectedFiles (for photo upload)
          await window.TelegramConnect.sendDocument(selectedFiles, "Accomplishment Report", selectedFiles);
          console.log("Telegram notification sent successfully.");
        } catch (telegramError) {
          console.error("Telegram send failed (non-blocking):", telegramError);
          // Optional: Show warning but don't block success
          if (window.Modal && window.Modal.show) {
            window.Modal.show("Details saved, but Telegram notification failed. Check console.", "warning");
          }
        }
      } else {
