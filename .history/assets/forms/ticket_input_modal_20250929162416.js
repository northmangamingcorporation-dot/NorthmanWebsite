 an// Enhanced assets/js/ticket_input_modal.js - Modern Ticket Input Form
    ...
    return { data: ticketData, valid: true };
      ...
      rawInput.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' && !e.shiftKey) {
          e.preventDefault();
          if (validateForm() && rawInput.value.trim()) {
      ...
      form.addEventListener('submit', async (e) => {
        ...
        // Show processing message and loading
        ...
        try {
          ...
          if (referenceCode && teller) {
            const duplicateSnapshot = await window.db.collection('it_requests')
              .where('parsedData.reference_code', '==', referenceCode)
              .where('parsedData.teller', '==', teller)
              .get();
            if (!duplicateSnapshot.empty) {
              throw new Error('A ticket with the same Reference Code and Teller already exists.');
            }
          }
