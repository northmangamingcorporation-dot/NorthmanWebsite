...
function attachTravelOrderForm(user) {
  ...
  waitForElements(({ modal, form, loadingOverlay, submitBtn, addPassengerBtn, passengersWrapper, closeBtn, cancelBtn, passengersError }) => {
    ...
    // Full field validation function
      ...
      try {
        ...
        for (let [key, value] of formData.entries()) {
          if (key === 'passenger[]') {
            ...
            if (value.trim()) data.passengers.push(value.trim());
          } else {
            data[key] = value;
          }
        }
        ...
        // Success handling
        console.log("Travel order submitted successfully");
        showError("Travel order submitted successfully!", "success");
        closeModal();

      } catch (error) {
        console.error("Submission error:", error);
        showError("Failed to submit travel order. Please try again.", "error");
      } finally {
...
// Main function to open the travel order modal
function openTravelOrderModal(user) {
  console.log("Opening travel order modal...");
  ...
  // Render and inject the modal
  document.body.insertAdjacentHTML('beforeend', renderTravelOrderForm(user));
