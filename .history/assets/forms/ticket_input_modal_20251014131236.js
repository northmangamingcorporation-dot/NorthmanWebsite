  ...
  window.db.collection('ticket_humanErr_report')
    ...
    .onSnapshot((snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        // Trim to avoid "DDN-311" vs "DDN-311  "
        const teller = (ticket.parsedData?.teller || ticket.teller || 'Unknown').toString().trim();
...
window.sortTicketsBy = sortTicketsBy
window.clearTicketFilters = clearTicketFilters
window.updateTicketTotal = updateTicketTotal
window.updateTicketsDateDisplay = updateTicketsDateDisplay
  ...
  window.db.collection('ticket_humanErr_report')
    ...
    .onSnapshot((snapshot) => {
      if (snapshot.empty) {
        rankingsList.innerHTML = `
          <div class="empty-state" style="text-align:center; padding:40px 20px; color:#64748b;">
            ...
            <p style="margin:0; font-size:16px;">No teller data available yet</p>
          </div>
        `;
        return;
      }
      ...
      if (sortedTellers.length === 0) {
        rankingsList.innerHTML = `
          <div class="empty-state" style="text-align:center; padding:40px 20px; color:#64748b;">
            <i class="fas fa-filter" style="font-size:48px; color:#cbd5e1; margin-bottom:12px;"></i>
            <p style="margin:0; font-size:16px;">No data found for selected period</p>
          </div>
        `;
        return;
      }
...
// Update filter and refresh rankings
function updateFilter(filterType) {
  ...
  // Refresh rankings (the listener will automatically apply the new filter)
...
function showCustomDatePicker() {
  ...
  modal.className = 'custom-date-modal-overlay';
  modal.onclick = function() { this.remove(); };
  
  modal.innerHTML = `
    <div class="custom-date-modal" onclick="event.stopPropagation()">
      <h3>
      ...
      <div class="modal-actions">
        <button class="modal-btn modal-btn-cancel" onclick="this.closest('.custom-date-modal-overlay').remove()">
          Cancel
        </button>
        <button class="modal-btn modal-btn-apply" onclick="applyCustomDateRange()">
          Apply
        </button>
      </div>
    </div>
...
async function initializeErrorChartModal(tellerName) {
  ...
  snapshot.forEach(doc => {
    ...
    if (typeof submittedAt === "string") submittedAt = new Date(submittedAt);
    else if (submittedAt.toDate) submittedAt = submittedAt.toDate();

    tellerDates.push(submittedAt);

    // Normalize to YYYY-MM-DD so each day is unique
    const dayKey = submittedAt.toISOString().split("T")[0]; 
  ...
  // Find first and last date to cover entire range
  tellerDates.sort((a, b) => a - b);
  const startDate = tellerDates[0];
  const endDate = tellerDates[tellerDates.length - 1];

  // Build all dates from start to end
  const labels = [];
  const values = [];
