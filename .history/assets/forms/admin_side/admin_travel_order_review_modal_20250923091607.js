...
async function loadDepartmentStaff() {
  try {
    ...
    const adminDoc = await db.collection("users").doc(window.currentUser.uid).get();
    ...
    // Query staff from same department
    ...
    select.innerHTML = `<option value="">-- Select Staff --</option>`; // reset

    staffSnapshot.forEach(doc => {
      const staff = doc.data();
      const option = document.createElement("option");
      option.value = staff.fullName || staff.username || doc.id;
      option.textContent = staff.fullName || staff.username || `Staff (${doc.id})`;
      select.appendChild(option);
    });

    // Fill department field automatically
...
// --- Mount function ---
async function mountAdminTravelOrderModal(admi,orderData) {
  ...
  // Load staff for this admin’s department
