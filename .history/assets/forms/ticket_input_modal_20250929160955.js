...
function formatDateForInput(date) {
  ...
  function showTicketInputModal(user = window.currentUser  ) {
    ...
    // Remove existing ticket modal if present
  ...
  function initializeTicketInputForm(user) {
    // Wait for elements to be ready (similar to Travel Order)
    ...
    // Helper function to show errors (fallback to alert if no Modal)
  ...
  // Enhanced real-time parsing (updated processTicketInput logic)
  function parseTicketInput(rawText) {
    ...
    for (let i = 0; i < lines.length - 1; i++) {
      ...
      // Check if current looks like a label (no special chars, short) and next looks like a value (has special chars or longer)
      if (/^[A-Za-z\s]+$/.test(current) && current.length < 50 && (next.includes('-') || next.length > 10 || /\d/.test(next))) {
        isStructured = true;
        break;
      }
    }
    ...
    if (isStructured) {
      ...
      lines.forEach((line) => {
        ...
        if (colonMatch) {
          ...
          // If no clear key-value, treat as description
    ...
    // Validate parsed data (require at least essential fields)
    const hasEssentialData = ticketData.ticket_code || ticketData.transaction_id || ticketData.type || ticketData.description || Object.keys(ticketData).length > 0;
    if (!hasEssentialData) {
    ...
    return { data: ticketData, valid: true };
      ...
      rawInput.addEventListener('input', debounce((e) => {
        ...
        // Store parsed data in a form field (hidden) for submission
        const hiddenData = document.createElement('input');
        hiddenData.type = 'hidden';
        hiddenData.name = 'parsedTicketData';
        hiddenData.value = valid ? JSON.stringify(data) : '{}';
        if (!form.querySelector('[name="parsedTicketData"]')) {
          form.appendChild(hiddenData);
        } else {
          form.querySelector('[name="parsedTicketData"]').value = valid ? JSON.stringify(data) : '{}';
        }
        validateForm();
      }, 500));
      ...
      function validateForm() {
        ...
        const hasValidParsedData = parsedDataInput && JSON.parse(parsedDataInput.value || '{}').valid !== false;
        ...
        if (isValid && rawInput.value.trim() && hasValidParsedData) {
          ...
          submitBtn.title = 'Ticket is ready to submit.';
        } else {
      ...
      // Initial validation and counter
      updateCharCounter();
      validateForm();
      ...
      form.addEventListener('submit', async (e) => {
        ...
        const parsedDataInput = form.querySelector('[name="parsedTicketData"]');
        const parsedTicketData = JSON.parse(parsedDataInput.value || '{}');
        ...
        try {
          ...
          const ticketCode = parsedTicketData.data.ticket_code;
          const transactionId = parsedTicketData.data.transaction_id;
          if (ticketCode || transactionId) {
            const query = window.db.collection('it_requests');
            let duplicateQuery = query;
            if (ticketCode) {
              duplicateQuery = duplicateQuery.where('parsedData.ticket_code', '==', ticketCode);
            } else if (transactionId) {
              duplicateQuery = duplicateQuery.where('parsedData.transaction_id', '==', transactionId);
            }
            const duplicateSnapshot = await duplicateQuery.get();
            if (!duplicateSnapshot.empty) {
              throw new Error('A ticket with the same Ticket Code or Transaction ID already exists.');
            }
          }

          // Save to Firestore
          await window.db.collection('it_requests').add(ticketData);
          ...
          setTimeout(() => {
            ...
            closeModal();
            // Refresh IT requests table if function exists
            if (typeof loadITRequests === 'function') loadITRequests();
            // Or update DOM table directly
            if (typeof addITRequest === 'function') addITRequest(ticketData);
          }, 2000);
  ...
  function addITRequest(ticketData) {
    // If Firestore data is passed, use it; otherwise assume DOM-only
    ...
    const type = ticketData.parsedData?.type || 'Ticket';
    const details = ticketData.description || ticketData.parsedData?.remarks || 'N/A';
    ...
    `;
    tbody.insertBefore(row, tbody.firstChild); // Add to top for newest first
  ...
  function viewITRequestDetails(id) {
    ...
    // Fetch ticket data from Firestore or local storage
    // Assuming window.db is available and tickets are stored in 'it_requests'
    if (!window.db) {
    ...
    window.db.collection('it_requests').doc(id).get().then((doc) => {
      if (doc.exists) {
  ...
  function showTicketDetailsModal(ticketData) {
    ...
    // Remove existing details modal if present
