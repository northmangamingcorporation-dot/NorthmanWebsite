...
function attachAccomplishmentForm(user) {
  ...
  function setupDragAndDrop() {
    ...
    // NEW: Paste image support (Ctrl+V)
...
function handlePaste(e) {
  ...
  if (files.length > 0) {
    ...
    // Process pasted files (same as drop/click)
    ...
    dropZone.innerHTML = `
      ...
      <p style="margin: 0; color: #10b981; font-weight: 500; font-size: 16px;">${files.length} image(s) pasted successfully!</p>
      <small style="color: #9ca3af; font-size: 12px;">Drop more, click to browse, or paste again (Ctrl+V)</small>
      <div id="dropZoneError" style="color: #ef4444; font-size: 14px; margin-top: 8px; display: none;"></div>
    `;
    setTimeout(() => {
      // Reset drop zone UI after 2s
      dropZone.innerHTML = `
        <i class="fas fa-cloud-upload-alt" style="font-size: 48px; color: #94a3b8; transition: color 0.3s ease;"></i>
        <p style="margin: 0; color: #64748b; font-weight: 500; font-size: 16px;">Drop images here or <span style="color: #0ea5a4; text-decoration: underline; cursor: pointer;">click to browse</span></p>
        <small style="color: #9ca3af; font-size: 12px;">Upload up to 5 image files (max 5MB each) as proof of accomplishment. Supported: JPG, PNG, GIF.</small>
        <div id="dropZoneError" style="color: #ef4444; font-size: 14px; margin-top: 8px; display: none;"></div>
      `;
      // Re-attach click listener after reset
      dropZone.addEventListener('click', () => fileInputHidden.click());
    }, 2000);
  } else if (items.length > 0 && !files.length) {
    showDropError('Pasted content is not a valid image or too large. Try copying an image (e.g., screenshot).');
  }
}

// NEW: Focus event for paste hint (shows on click/focus)
dropZone.addEventListener('focusin', () => {
  // Add temporary hint if not dragging
  if (!dropZone.classList.contains('drag-over')) {
    const hint = document.createElement('div');
    hint.style.cssText = 'position: absolute; top: 10px; left: 50%; transform: translateX(-50%); background: rgba(14, 165, 164, 0.1); color: #0ea5a4; padding: 4px 8px; border-radius: 4px; font-size: 12px; z-index: 1;';
    hint.textContent = 'Paste images with Ctrl+V';
...
function processFiles(files) {
  ...
  if (duplicates.length > 0) {
    showDropError(`Duplicate image(s) detected: ${duplicates.map(f => f.name).join(', ')}. Skipping.`);
    validFiles = validFiles.filter(f => !duplicates.includes(f));
  }

  if (validFiles.length > 0) {
    selectedFiles = [...selectedFiles, ...validFiles];
    fileInputHidden.files = new DataTransfer().files; // Clear hidden input
    renderPreviews();
    validateForm();
    clearDropError();
  }
