          ...
          <section class="form-section">
            ...
            ">
              ...
              <div class="form-group" style="display: flex; flex-direction: column; grid-column: 1 / -1;">
                ...
                <div class="field-error" style="color: #ef4444; font-size: 14px; margin-top: 4px;"></div>
              </div>
...
// Enhanced attach logic with validation, Firebase integration, drag-and-drop, and image preview/zoom
function attachAccomplishmentForm(user) {
  ...
  function setupDragAndDrop() {
    ...
    function processFiles(files) {
      ...
      fileInputHidden.files = new DataTransfer().files; // Clear hidden input, we'll use selectedFiles for submission
      renderPreviews();
  ...
  function renderPreviews() {
    ...
    // Update hidden input with selected files for form submission
  ...
  window.openImageView = function(src, filename) {
    ...
    window.closeImageView = function() {
      ...
      // Clean up event listeners if needed (for simplicity, they persist but only active when modal open)
    };
    ...
    // Touch support (basic pinch zoom - enhanced libraries like Hammer.js could be used for full gesture)
  ...
  // Form submission with file uploads (using selectedFiles)
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // Handle file uploads to Firebase Storage using selectedFiles
          ...
          uploadPromises.push(
            new Promise((resolve, reject) => {
              uploadTask.on(
                'state_changed',
                (snapshot) => {
                  // Progress can be monitored here if needed (e.g., update UI)
                  const progress = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
                  console.log(`Upload ${i + 1}/${selectedFiles.length}: ${progress.toFixed(0)}%`);
                },
                (error) => reject(error),
                async () => {
                  try {
