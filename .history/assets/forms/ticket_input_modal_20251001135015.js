...
function updateTicketsTotal() {
  ...
  const totalRow = document.getElementById("ticketsTotalRow");
  const total = tbody.querySelectorAll("tr:not(.no-data-row)").length;
  if (totalRow) totalRow.textContent = `Total Tickets: ${total}`;
}
