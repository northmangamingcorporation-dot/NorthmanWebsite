...
function renderTellerRankingItem(rank, teller, count, isTopError = false) {
  ...
  return `
    ...
    " onmouseover="this.style.transform='translateX(4px)'" onmouseout="this.style.transform='translateX(0)'" onClick="showErrorChartModal(tellerName)">
...
// Show Error Chart Modal for ONE teller
function showErrorChartModal(tellerName) {
  ...
  const html = renderErrorChartModal();
  document.body.insertAdjacentHTML("beforeend", html);

  // Initialize chart after rendering
  setTimeout(() => initializeErrorChartModal(tellerName), 100);

  console.log("Error Chart Modal mounted successfully for Teller:", tellerName);
}
