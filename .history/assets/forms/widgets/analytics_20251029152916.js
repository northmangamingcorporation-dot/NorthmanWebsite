...
(function() {
    ...
    const state = {
        ...
        connectionMode: 'disconnected', // disconnected, connecting, events, polling
        consecutiveErrors: 0,
        clientId: null
    };
    ...
    async function executeQuery(query, params = []) {
        try {
            ...
            logger.info(`Query returned ${data.results?.length || 0} results`);
            return data.results || [];
        } catch (error) {
    ...
    async function fetchAnalyticsData() {
        // Prevent concurrent fetches (anti-spam)
        if (state.loading) {
            ...
            // Return empty structure to prevent null errors
            return {
                cancellations: {
                    total: 0,
                    last_24h: 0,
        ...
        if (timeSinceLastFetch < 5000 && state.lastSuccessfulData) {
            logger.warn(`Rate limited: ${timeSinceLastFetch}ms since last fetch, using cache`);
            return state.lastSuccessfulData;
        }
        ...
        try {
            ...
            logger.info(`✅ Analytics data fetched (${data.cancellations.total} cancellations, ${data.payouts.total} payouts)`);
            return data;
    ...
    function startEventStream() {
        ...
        try {
            // EventSource doesn't support custom headers, so pass API key as URL parameter
            state.eventSource = new EventSource(
                `${CONFIG.API_URL}/api/events/subscribe?api_key=${CONFIG.API_KEY}`,
                { 
            ...
            state.eventSource.addEventListener('message', (event) => {
                try {
                    ...
                    switch(data.type) {
                        case 'connected':
                            logger.info(`📱 Connected as client: ${data.client_id}`);
                            state.clientId = data.client_id;
                            updateConnectionStatus();
                            break;
                            
                        case 'update':
                            logger.info(`🔄 Update received: ${data.channel || 'general'}`);
                        ...
                        default:
                            logger.info(`📨 Event received: ${data.type}`);
                    }
                } catch (error) {
            ...
            state.eventSource.onerror = (error) => {
                ...
                if (state.eventSource.readyState === EventSource.CLOSED) {
                    ...
                    const backoffDelay = Math.min(5000 * Math.pow(2, state.consecutiveErrors - 1), 30000);
                    
                    setTimeout(() => {
                        if (state.consecutiveErrors < 5) {
                            logger.info(`🔄 Reconnecting in ${backoffDelay}ms...`);
                            startEventStream();
                        } else {
                ...
                } else if (state.eventSource.readyState === EventSource.CONNECTING) {
                    logger.info('🔄 Event stream reconnecting...');
                    state.connectionMode = 'connecting';
                    updateConnectionStatus();
                }
            };
    ...
    function updateConnectionStatus() {
        ...
        const statusMap = {
            ...
            connecting: { text: '🟡 Connecting', class: 'status-polling', icon: 'fa-circle-notch fa-spin' },
        ...
        // Add client ID if connected via SSE
        if (state.connectionMode === 'events' && state.clientId) {
            statusEl.title = `Client ID: ${state.clientId}`;
        }
    }
    ...
    function renderHeader() {
        ...
        const dateRangeLabel = safeGet(state, 'lastSuccessfulData.dateRange', '');
            ...
            <div class="analytics-header">
                <div class="header-left">
                    ...
                    <p class="header-subtitle">
                        ...
                        ${dateRangeLabel ? `<span class="separator">•</span><i class="fas fa-calendar"></i> ${dateRangeLabel}` : ''}
                    </p>
                </div>
    ...
    function renderSummaryCards(data) {
        ...
        const dateRange = safeGet(data, 'dateRange', 'Last 30 days');
            ...
            <div class="summary-cards">
                <div class="summary-card">
                    ...
                    <div class="summary-content">
                        ...
                        <div class="summary-change">${dateRange}</div>
                    </div>
                </div>
                ...
                <div class="summary-card">
                    ...
                    <div class="summary-content">
                        ...
                        <div class="summary-change success">${dateRange}</div>
                    </div>
                </div>
            </div>
        `;
    ...
    async function initialize() {
        ...
        // If already loaded, just update the data without full re-init
        if (state.loaded) {
            ...
            try {
                ...
                // Update only the content, not the header/tabs
                ...
                if (summaryDiv && summaryDiv.parentElement) {
                    ...
                    newSummary.innerHTML = renderSummaryCards(data);
                    summaryDiv.parentElement.replaceChild(newSummary.firstElementChild, summaryDiv);
                }
                
                // Update header to show new timestamp
                const header = document.querySelector('.analytics-header');
                if (header) {
                    header.outerHTML = renderHeader();
                    updateConnectionStatus();
    ...
    function refresh() {
        ...
        // Don't reset loaded flag - we want to keep the UI
        // state.loaded = false; // ← Removed this line
        return initialize();
    }
