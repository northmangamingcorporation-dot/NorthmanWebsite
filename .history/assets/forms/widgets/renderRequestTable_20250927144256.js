// REQUIRED DEPENDENCIES: Define these before calling loadOrders (or include them in your script)
// Firestore collection reference (define once, e.g., in global scope or attachDashboard)
const ordersCol = window.db.collection("it_service_orders");
let unsubscribe = null;  // For listener cleanup (global or outer scope)

// Helper: getStatusColor (returns colors for status badges)
...
// Helper: updateStats (updates dashboard stat counters with animation)
...
// Helper: getCurrentStats (retrieves current stat values from DOM)
...
// Helper: hideLoading (removes the loading overlay)
...
// MAIN FUNCTION: loadOrders(user) - Loads IT service orders table with real-time data
// Features: High-contrast colors, injected styles, loading overlay, animations, status badges, cancel buttons with Firestore sync.
  ...
  unsubscribe = ordersCol
    ...
    .onSnapshot(
      async (snapshot) => {
        ...
        // Attach cancel button events (after rows are rendered)
        document.querySelectorAll(".btn-cancel").forEach(btn => {
          // Remove existing listeners to avoid duplicates
          btn.replaceWith(btn.cloneNode(true));
          const newBtn = btn.closest('tr').querySelector('.btn-cancel');
          newBtn.addEventListener("click", async (e) => {
            ...
            try {
              ...
              // Update local row immediately for UX
