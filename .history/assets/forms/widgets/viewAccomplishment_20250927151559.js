...
overlay.addEventListener('click', (e) => {
  ...
  // FIXED: Update closeModal to include lightbox cleanup (now safe with 'let')
...
photoItems.forEach((item, index) => {
  ...
  item.addEventListener('click', (e) => {
    ...
    tempImg.onload = () => {
      ...
      img.src = fullImgSrc;  // Set src in lightbox img element
      img.alt = `Zoomed evidence ${index + 1}`;
      resetZoom();  // Reset state for new image (scale=1, translate=0)
      lightbox.classList.add('active');  // Open lightbox
      console.log(`Lightbox opened with src: ${fullImgSrc}`);
    };
    tempImg.onerror = () => {
      console.error(`Failed to load image: ${fullImgSrc}`);
      // Fallback placeholder SVG (centered error message)
    ...
    tempImg.src = fullImgSrc;  // Trigger load
  });
});
