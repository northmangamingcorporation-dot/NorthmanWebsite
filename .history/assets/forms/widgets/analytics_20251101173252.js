...
async function fetchDataGatheringRate(filterMode = 'day') {
    ...
    .overview-header h3, .rankings-header h3, .tab-header h3 {
        font-size: 20px;
        font-weight: 700;
        margin: 0 0 8px;
        display: flex;
    ...
    case 'rankings':
        ...
        break;
    case 'cancellations':
        fetchComprehensiveData(state.filters.filterMode).then(comprehensiveData => {
            const cancellationData = {
                total: (comprehensiveData.metrics?.cancellations?.total_pending || 0) + 
                       (comprehensiveData.metrics?.cancellations?.total_approved || 0) + 
                       (comprehensiveData.metrics?.cancellations?.total_denied || 0) + 
                       (comprehensiveData.metrics?.cancellations?.total_forced || 0),
                last_24h: 0,
                by_status: {
                    approved: comprehensiveData.metrics?.cancellations?.total_approved || 0,
                    denied: comprehensiveData.metrics?.cancellations?.total_denied || 0,
                    pending: comprehensiveData.metrics?.cancellations?.total_pending || 0,
                    forced: comprehensiveData.metrics?.cancellations?.total_forced || 0
                },
                approval_rate: 0,
                top_operators: [],
                trend: []
            };
            renderCancellationAnalytics(cancellationData, contentDiv);
        }).catch(error => {
    ...
    case 'payouts':
        fetchComprehensiveData(state.filters.filterMode).then(comprehensiveData => {
            const payoutData = {
                total: comprehensiveData.metrics?.payouts?.total_count || 0,
                total_amount: comprehensiveData.metrics?.payouts?.total_amount || 0,
                average_amount: comprehensiveData.metrics?.payouts?.total_count > 0 
                    ? (comprehensiveData.metrics?.payouts?.total_amount / comprehensiveData.metrics?.payouts?.total_count) 
                    : 0,
            ...
            renderPayoutAnalytics(payoutData, contentDiv);
        }).catch(error => {
    ...
    case 'device-changes':
        fetchComprehensiveData(state.filters.filterMode).then(comprehensiveData => {
            const deviceData = {
                total: comprehensiveData.metrics?.other?.total_device_changes || 0,
                last_24h: 0,
                by_type: {
                    phone: 0,
                    pos: 0
            ...
            renderDeviceChangeAnalytics(deviceData, contentDiv);
        }).catch(error => {
