// UPDATED: Load requests with enhanced design matching IT Service Orders
...
// status badges with colors, graceful error handling, integrated with mountRequestModal
// Assumes: unsubscribe variable (global or outer scope), window.Modal for confirm/show (fallback to alert/confirm),
// getStatusColor function (defined below), table with id="requestsTable" and 6 columns (ID, Description, Date, Status, Photos, Action)

let unsubscribeRequests; // Global unsubscribe for real-time listener (parallel to unsubscribe in loadOrders)
...
async function loadRequests(currentUser ) {
  if (unsubscribeRequests) unsubscribeRequests(); // Clean up previous listener

  console.log("Loading requests for:", currentUser .username);

  // Real-time listener (matching loadOrders)
  unsubscribeRequests = window.db.collection('requests')
    ...
    .onSnapshot(
      async (snapshot) => {
          ...
          tbody.innerHTML = `
            <tr class="no-data-row">
              <td colspan="6" style="background: white; padding: 60px 20px;">
                ...
                ">
                  <i class="fas fa-clipboard-list" style="font-size: 64px; color: #94a3b8;"></i>
                  <h3 style="margin: 0; color: #1e293b; font-size: 20px; font-weight: 600;">No Requests Yet</h3>
                  <p style="margin: 0; color: #64748b; font-size: 16px; font-style: italic;">Submit your first request to get started!</p>
                </div>
              </td>
            </tr>
          `;
        ...
        snapshot.forEach(doc => {
          ...
          console.log("Request loaded:", doc.id, request);
          requests.push(request);
        });

        // Render rows with animations and high-contrast text
        requests.forEach(request => {
          const createdAt = request.dateSubmitted?.toDate
            ? request.dateSubmitted.toDate().toLocaleDateString()
            : 'N/A';

          const statusKey = (request.status || 'submitted').toLowerCase();
          const statusColor = getStatusColor(statusKey);

          const photoCount = request.telegramPhotos ? request.telegramPhotos.length : 0;
          ...
          row.innerHTML = `
            <td style="padding: 16px 12px; font-weight: 600; color: #0f172a !important;">
              <div style="display: flex; align-items: center; gap: 8px;">
                ...
                <span style="color: #0f172a;">${request.id}</span>
              </div>
            </td>
            <td style="padding: 16px 12px; color: #475569 !important; max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
              <span title="${request.descriptionOfService || 'N/A'}">${request.descriptionOfService?.substring(0, 50) || 'N/A'}...</span>
            </td>
            ...
            <td style="padding: 16px 12px;">
              <button onclick="mountRequestModal('${request.id}')" 
                      style="
                        padding: 8px 16px;
                        font-size: 14px;
          ...
          }, requests.indexOf(request) * 100);
        });

        // Fade in table body after rows are added
        setTimeout(() => {
          tbody.style.opacity = "1";
        }, requests.length * 100 + 200);

      },
      (err) => {
        hideLoading('dashboard-loading-requests');
        console.error("Firestore snapshot error for requests:", err);
        const tbody = document.getElementById('requestsTable');
        tbody.innerHTML = `
          <tr class="no-data-row">
