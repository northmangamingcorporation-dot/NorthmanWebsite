...
function attachAccomplishmentModal() {
  ...
  // Inject enhanced styles (once, for gallery, drag, zoom like IT Service Order) - Updated for formal design and zoom icon
  if (!document.querySelector('#dynamic-accomplishment-styles')) {
    ...
    style.textContent = `
      ...
      .photo-preview-item:hover .zoom-icon-overlay { display: flex !important; }
      .zoom-icon-overlay { 
        display: none; 
        opacity: 0; 
        transition: opacity 0.2s ease; 
      }
      .photo-preview-item:hover .zoom-icon-overlay { opacity: 1; }
  ...
  // Photo Zoom Functionality (lightbox on click, like IT Service Order Modal) - Enhanced with zoom icon visibility
