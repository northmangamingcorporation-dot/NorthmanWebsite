...
// UPDATED: mountAccomplishmentModal(id) now fully handles fetching, rendering, and attaching the modal in one call.
//          Returns a Promise that resolves when modal is shown (or rejects on error). Call directly on click: mountAccomplishmentModal('some-id').
//          Optional pre-fetched data still supported for testing/offline use (skips fetch, directly shows modal).

async function mountAccomplishmentModal(id, optionalPreFetchedData = null) {
  // Standalone: Allow passing data directly for testing or if db unavailable (directly render and attach)
  if (optionalPreFetchedData) {
    ...
    attachAccomplishmentModal(mountedData);
    return Promise.resolve(mountedData);  // Resolve with data after attaching
  }
  ...
  if (!id) {
    ...
    if (window.Modal && window.Modal.show) {
      ...
      alert('Error: Invalid report ID. For standalone testing, pass pre-fetched data as second argument.');
    }
  ...
  try {
    ...
    // Step 1: Fetch full doc from Firestore (if db available)
    let data = null;
    let photos = [];
    if (!window.db) {
    ...
    // Step 3: Prepare mounted data and automatically render + attach the modal
    const mountedData = {
      data,
      photos,
      id
    ...
    // Return the mounted data (for optional further use, e.g., logging)
    return Promise.resolve(mountedData);
      ...
      <div style="background: #ffffff; padding: 25px; border-radius: 0 0 12px 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 15px; margin-bottom: 25px;">
          ...
          <div style="background: #f8fafc; padding: 15px; border-radius: 8px; border-left: 4px solid #10b981;">
            ...
            <p style="color: #374151; margin-top: 5px;">${data.dateSubmitted ? new Date(data.dateSubmitted).toLocaleString() : 'N/A'}</p>
          </div>
        ...
        ${Object.entries(data).filter(([key]) => !['id', 'uniquekey', 'status', 'dateSubmitted', 'updatedAt', 'telegramPhotos'].includes(key)).map(([key, value]) => {
          const val = (value && typeof value.toDate === 'function') ? value.toDate().toLocaleString() : value;
          return `
    ...
    photos.slice(0, 12).forEach((photo, index) => {  // Limit to 12 for better layout
      ...
      contentHTML += `
        <div class="photo-preview-item" style="position: relative; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 8px rgba(0,0,0,0.1); transition: transform 0.2s; cursor: pointer;">
          ...
          <img src="${fullSrc}" alt="Full Photo ${index + 1}" class="hidden-full-img" style="display: none;" data-index="${index}">
        </div>
      `;
    });
    if (photos.length > 12) {
...
// Function to attach and display the modal (with drag, zoom, and event handling) - unchanged, called internally by mountAccomplishmentModal
...
// Or async: await mountAccomplishmentModal('some-id'); // Resolves after modal is shown
