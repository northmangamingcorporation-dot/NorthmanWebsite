...
async function viewAccomplishment(id) {
  ...
  // Clean up any existing lightbox/openLightbox function
  const existingLightbox = document.querySelector('.lightbox-overlay');
  if (existingLightbox) existingLightbox.remove();
  if (window.openLightbox) delete window.openLightbox;
  ...
  try {
    ...
    // Prepare full valid photos for lightbox (filter broken, no slice for full access)
    ...
    if (validPhotos.length > 0) {
      contentHTML += `
        <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
          <h4 style="color: #0ea5a4; margin: 0 0 15px; font-size: 18px;">📸 Proof Photos (${validPhotos.length})</h4>
      ...
      validPhotos.slice(0, 10).forEach((photo, index) => {
        const thumbSrc = photo.thumbnail || photo.url;  // Fallback to main if no thumb
        contentHTML += `
    ...
    if (!document.querySelector('#enhanced-modal-styles')) {
      ...
      style.textContent = `
        ...
        .lightbox-overlay { 
          position: fixed; top: 0; left: 0; width: 100%; height: 100%; 
          background: rgba(0,0,0,0.95); z-index: 10001; display: flex; align-items: center; justify-content: center;
          opacity: 0; transition: opacity 0.3s ease; padding: 20px;
        }
        .lightbox-overlay.show { opacity: 1; }
        .lightbox-container { 
          position: relative; width: 90vw; height: 90vh; max-width: 1200px; max-height: 800px; 
          display: flex; align-items: center; justify-content: center; background: #000;
          border-radius: 8px; overflow: hidden;
        }
        .lightbox-image { 
          max-width: 100%; max-height: 100%; object-fit: contain; cursor: grab; 
          transition: transform 0.3s ease; transform-origin: center center;
          user-select: none; -webkit-user-drag: none;
        }
        .lightbox-image.zoomed { cursor: grabbing; }
        .lightbox-caption { 
          position: absolute; bottom: 20px; left: 50%; transform: translateX(-50%); 
          background: rgba(0,0,0,0.7); color: white; padding: 10px 15px; border-radius: 6px; 
          max-width: 80%; font-size: 14px; text-align: center; opacity: 0; transition: opacity 0.3s;