      ...
      <div style="background: #ffffff; padding: 25px; border-radius: 0 0 12px 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 15px; margin-bottom: 25px;">
          ...
          <div style="background: #f8fafc; padding: 15px; border-radius: 8px; border-left: 4px solid #10b981;">
            ...
            <p style="color: #374151; margin-top: 5px;">${data.dateSubmitted ? new Date(data.dateSubmitted).toDate ? new Date(data.dateSubmitted).toDate().toLocaleString() : new Date(data.dateSubmitted).toLocaleString() : 'N/A'}</p>
          </div>
...
function attachAccomplishmentModal() {
  ...
  if (!document.querySelector('#dynamic-accomplishment-styles')) {
    ...
    style.textContent = `
      ...
      .dragging * { cursor: grabbing !important; user