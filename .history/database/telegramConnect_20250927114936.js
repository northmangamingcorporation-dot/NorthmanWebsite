...
window.TelegramConnect = (function () {
  ...
  async function SendPhotos(photos, id, requestType) {
    ...
    const id = formData.id || `AC-${Date.now()}`;
    await window.db.collection('accomplishments').doc(id).set({
      // ... your docData (id, descriptionOfService, status, dateSubmitted, etc.)
    }, { merge: true });
    let telegramPhotosStored = [];  // Optional: Track for logging
    // Your existing photo handling (unchanged)
    if (photos && photos.length > 0) {
      const sentPhotos = await SendPhotos(photos, id, requestType);  // Already returns [{file_id, caption, message_id}, ...]
      // ADD THIS: Update Firestore with the full sentPhotos array (includes message_id)
      if (sentPhotos.length > 0 && window.db) {
        try {
          await window.db.collection('accomplishments').doc(id).update({
            telegramPhotos: firebase.firestore.FieldValue.arrayUnion(...sentPhotos)  // Appends objects with message_id
          });
          telegramPhotosStored = sentPhotos;
          console.log(`Stored ${sentPhotos.length} telegramPhotos with message_ids in Firestore for ID: ${id}`);
        } catch (updateErr) {
          console.error('Failed to update telegramPhotos in Firestore:', updateErr);
          // Optional: Fallback to set if arrayUnion fails (e.g., new doc)
          await window.db.collection('accomplishments').doc(id).set({
