...
window.TelegramConnect = (function () {
  ...
  const mentions = {
    ...
    "Travel Order": ["@MrCooperJosh", "@kimnanaeh", "@KeCab_23"],
    "Driver's Trip Ticket": ["@MrCooperJosh", "@kimnanaeh", "@KeCab_23"],
    "Accomplishment Report": ["@MrCooperJosh", "@kimnanaeh", "@KeCab_23", "@Yong1961", "@Kaloiskie_IT", "@Kristinemdt"],  
    "Early Rest Day": ["@Yannie_me"],
    "Leave Request": ["@Yannie_me"]// Add users if needed, e.g., ["@admin1"]
  };
...
// Helper: Format key to title case with spaces (e.g., "driversName" → "Drivers Name")
function formatKey(key) {
  if (!key || typeof key !== 'string') return key; // Skip invalid keys
  
  return key
    .replace(/([A-Z])/g, ' $1')  // Insert space before capital letters (e.g., "driversName" → "drivers Name")
    .replace(/[_-]/g, ' ')       // Handle snake_case or hyphens (e.g., "date_filed" → "date filed")
  ...
  async function SendPhotos(photos, id, requestType) {
    ...
    // Improved storage (appends instead of overwrites, with error handling)
    if (sentPhotos.length > 0 && window.db) {
      try {
        // Prefer arrayUnion for appending (change .set to .update if doc exists)
        ...
        console.log(`Appended ${sentPhotos.length} telegramPhotos (with message_ids) to Firestore for ID: ${id}`);
      } catch (dbErr) {
        console.error(`Update failed for ID: ${id} (doc may not exist):`, dbErr);
        // Fallback: Create/set if needed
        try {
          await window.db.collection('accomplishments').doc(id).set({
            telegramPhotos: sentPhotos
          }, { merge: true });
          console.log(`Fallback: Set ${sentPhotos.length} telegramPhotos in Firestore for ID: ${id}`);
        } catch (setErr) {
          console.error(`Fallback set failed for ID: ${id}:`, setErr);
        }
      }
    }
    ...
    sendRequest: async function(dataObj, requestType, files = []) {
      ...
      for (const [key, value] of Object.entries(dataObj)) {
        if (key !== 'id' && key !== 'uniquekey' && key !== 'status' && key !== 'dateSubmitted' && key !== 'updatedAt' && key !== 'proofImages') {
          if (value == null || value === '') continue;  // Skip empty/null values
      
          let val = (value && value.toDate) ? value.toDate().toLocaleString('en-US', { 
            year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' 
          }) : value;  // Enhanced date formatting (e.g., "Sep 27, 2025, 12:00 PM")
          
          const formattedKey = formatKey(key);  // NEW: Format key (e.g., "driversName" → "Drivers Name")
          message += `*${escapeMarkdownV2(formattedKey)}:* ${escapeMarkdownV2(val.toString())}\n`;
          hasFields = true;
        }
      }
      const mentionList = mentions[requestType] ? [...mentions[requestType]] : [];

// ✅ Add department heads dynamically
if (["Early Rest Day", "Leave Request"].includes(requestType)) {
  if (dataObj.department === "IT") {
    mentionList.push("@Yong1961"); // IT Head
  } else if (dataObj.department === "Admin") {
    mentionList.push("@MrCooperJosh"); // Admin Head
  }
}
