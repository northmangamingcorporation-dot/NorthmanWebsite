...
// Each section has its own real-time loader (loadTravelOrders, loadITServiceOrders, loadTripTickets) with shared design patterns:
// - Loading overlay per section (or global; here per-section for specificity).
// - Animated rows, empty/error states with icons, high-contrast text, status badges.
// - Assumes similar field structures (e.g., dateSubmitted, status, description/type); adapt queries/filters as needed.
// - Stats: Kept for IT service orders (as original); added placeholder stats for others (extend with HTML elements like stat-travel-pending).
// - Actions: View buttons calling hypothetical modals (e.g., openTravelModal); cancel for pending IT only.
// - Unsubscribes: Separate for each listener to allow independent cleanup.
// - Assumes: window.Modal, Font Awesome, table HTML with IDs (travelTable, requestsTable, tripTicketsTable), getStatusColor extended.
// - Integration: Calls attachAccomplishmentsSection(user) first; initializes all loaders.
...
async function attachDashboard(user) {
  ...
  // Inject shared table styles for all sections (once)
  ...
  // Shared hideLoading function (accepts ID)
  ...
  // Shared getStatusColor (extended for all sections)
  function getStatusColor(status) {
    const colors = {
      ...
      default: { bg: 'rgba(107, 114, 128, 0.1)', text: '#374151' }
    };
    return colors[status] || colors.default;
  }

  // IT Service Orders Loader (updated with visibility fixes)
  function loadITServiceOrders(currentUser ) {
    ...
    loadingOverlay.style.cssText = `position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.8); backdrop-filter: blur(10px); display: flex; justify-content: center; align-items: center; z-index: 2000; opacity: 0; transition: opacity 0.3s ease;`;
    ...
    unsubscribeIT = ordersCol
      ...
      .onSnapshot(async (snapshot) => {
            ...
            <tr class="no-data-row">
              <td colspan="5" style="background: white; padding: 60px 20px;">
                <div style="display: flex; flex-direction: column; align-items: center; gap: 16px; color: #475569;">
                  ...
                  <p style="margin: 0; color: #64748b; font-size: 16px; font-style: italic;">Create your first IT service request!</p>
                </div>
              </td>
            </tr>
          `;
        ...
        orders.forEach((order, index) => {
          const createdAt = order.dateSubmitted?.toDate?.toLocaleString() || new Date().toLocaleString();
        ...
        // Attach cancel events (same as original)
        document.querySelectorAll(".btn-cancel").forEach(btn => {
          btn.addEventListener("click", async (e) => {
            e.stopPropagation();
            const id = btn.dataset.id;
            const confirmed = window.Modal?.confirm ? await window.Modal.confirm("Are you sure?") : confirm("Cancel?");
            if (!confirmed) return;

            try {
              btn.disabled = true;
              btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Cancelling...';
              await ordersCol.doc(id).update({ status: "cancelled", updatedAt: window.db.FieldValue.serverTimestamp() });
              
              const row = btn.closest("tr");
              row.cells[3].innerHTML = '<span class="status-badge" style="background: rgba(107, 114, 128, 0.1); color: #6b7280 !important; padding: 6px 12px; border-radius: 20px;">Cancelled</span>';
              ...
              window.Modal?.show ? window.Modal.show("Cancelled successfully.", "success") : alert("Cancelled.");
            } catch (err) {
              console.error(err);
              btn.disabled = false;
              btn.innerHTML = '<i class="fas fa-times" style="margin-right: 4px; color: #ef4444;"></i>Cancel';
              window.Modal?.show ? window.Modal.show("Failed to cancel.", "error") : alert("Failed.");
            }
          });
        });
      }, (err) => {
        hideLoading(loadingId);
        console.error("IT snapshot error:", err);
        // Error row similar to empty state
        document.getElementById("requestsTable").innerHTML = `
          <tr class="no-data-row"><td colspan="5" style="text-align: center; color: #ef4444; padding: 60px; background: white;">
            <i class="fas fa-exclamation-triangle" style="font-size: 48px; color: #ef4444; margin-bottom: 16px;"></i>
            <h3 style="color: #dc2626; font-size: 18px; font-weight: 600; margin: 0 0 8px;">Error Loading IT Requests</h3>
            <p style="color: #64748b; font-size: 16px; margin: 0;">Refresh the page.</p>
          </td></tr>
        `;
        window.Modal?.show("Error loading IT requests.", "error");
      });
  }

  // Travel Orders Loader (similar structure; adapt fields as needed)
  function loadTravelOrders(currentUser ) {
    if (unsubscribeTravel) unsubscribeTravel();
    
    const loadingId = "dashboard-loading-travel";
    // Similar loading overlay (teal spinner for theme)
    const loadingOverlay = document.createElement("div");
    loadingOverlay.id = loadingId;
    loadingOverlay.style.cssText = `position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.8); backdrop-filter: blur(10px); display: flex; justify-content: center; align-items: center; z-index: 2000; opacity: 0; transition: opacity 0.3s ease;`;
    loadingOverlay.innerHTML = `
    ...
    unsubscribeTravel = travelCol
      .where("username", "==", currentUser .username) // Assume username field; adjust if uniquekey or other
      .orderBy("dateSubmitted", "desc")
      .onSnapshot(async (snapshot) => {
        const tbody = document.getElementById("travelTable"); // Assume table ID
        if (!tbody) return;
            ...
            <tr class="no-data-row">
              <td colspan="5" style="background: white; padding: 60px 20px;">
                <div style="display: flex; flex-direction: column; align-items: center; gap: 16px; color: #475569;">
                  ...
                  <p style="margin: 0; color: #64748b; font-size: 16px; font-style: italic;">Submit your first travel request!</p>
                </div>
              </td>
            </tr>
          `;
          // Update travel stats if HTML elements exist (e.g., stat-travel-pending)
          updateSectionStats("travel", { pending: 0, approved: 0 }); // Placeholder
          hideLoading(loadingId);
          return;
        }
        ...
        orders.forEach((order, index) => {
          const createdAt = order.dateSubmitted?.toDate?.toLocaleString() || new Date().toLocaleString();
          const statusKey = (order.status || "pending").toLowerCase();
          // Update travel stats if applicable
          updateSectionStats("travel", { [statusKey]: (snapshot.metadata.fromCache ? 0 : 1) }); // Simplified; adjust
          const statusColor = getStatusColor(statusKey);
          ...
          row.innerHTML = `
            <td style="padding: 16px 12px; font-weight: 600; color: #0f172a !important;">
              <div style="display: flex; align-items: center; gap: 8px;">
                ...
                <span style="color: #0f172a;">${order.destination || order.type || "-"}</span> <!-- Adapt fields -->
              </div>
            </td>
            <td style="padding: 16px 12px; color: #475569 !important; max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
              <span title="${order.description || '-'}">${order.description?.substring(0, 50) || "-"}...</span>
        ...
        hideLoading(loadingId);
        console.error("Travel snapshot error:", err);
        document.getElementById("travelTable").innerHTML = `
          <tr class="no-data-row"><td colspan="5" style="text-align: center; color: #ef4444; padding: 60px; background: white;">
            <i class="fas fa-exclamation-triangle" style="font-size: 48px; color: #ef4444; margin-bottom: 16px;"></i>
            <h3 style="color: #dc2626; font-size: 18px; font-weight: 600; margin: 0 0 8px;">Error Loading Travel Orders</h3>
            <p style="color: #64748b; font-size: 16px; margin: 0;">Refresh the page.</p>
          </td></tr>
        `;
        window.Modal?.show("Error loading travel orders.", "error");
      });
  }

  // Drivers Trip Tickets Loader (similar structure; adapt fields)
  function loadTripTickets(currentUser  ) {
    ...
    unsubscribeTrip = tripCol
      .where("username", "==", currentUser  .username) // Assume username; adjust if needed
      .orderBy("dateSubmitted", "desc")
      .onSnapshot(async (snapshot) => {
        const tbody = document.getElementById("tripTicketsTable"); // Assume table ID
        if (!tbody) return;
            ...
            <tr class="no-data-row">
              <td colspan="5" style="background: white; padding: 60px 20px;">
                <div style="display: flex; flex-direction: column; align-items: center; gap: 16px; color: #475569;">
                  ...
                  <p style="margin: 0; color: #64748b; font-size: 16px; font-style: italic;">Log your first trip!</p>
                </div>
              </td>
            </tr>
          `;
          // Update trip stats if HTML elements exist
          updateSectionStats("trip", { pending: 0, completed: 0 }); // Placeholder
          hideLoading(loadingId);
          return;
        }
        ...
        tickets.forEach((ticket, index) => {
          const createdAt = ticket.dateSubmitted?.toDate?.toLocaleString() || new Date().toLocaleString();
          const statusKey = (ticket.status || "pending").toLowerCase();
          // Update trip stats
          updateSectionStats("trip", { [statusKey]: (snapshot.metadata.fromCache ? 0 : 1) });
          const statusColor = getStatusColor(statusKey);
          ...
          row.innerHTML = `
            <td style="padding: 16px 12px; font-weight: 600; color: #0f172a !important;">
              <div style="display: flex; align-items: center; gap: 8px;">
                ...
                <span style="color: #0f172a;">${ticket.vehicle || ticket.route || "-"}</span> <!-- Adapt fields -->
              </div>
            </td>
            <td style="padding: 16px 12px; color: #475569 !important; max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
        ...
        hideLoading(loadingId);
        console.error("Trip snapshot error:", err);
        document.getElementById("tripTicketsTable").innerHTML = `
          <tr class="no-data-row"><td colspan="5" style="text-align: center; color: #ef4444; padding: 60px; background: white;">
            <i class="fas fa-exclamation-triangle" style="font-size: 48px; color: #ef4444; margin-bottom: 16px;"></i>
            <h3 style="color: #dc2626; font-size: 18px; font-weight: 600; margin: 0 0 8px;">Error Loading Trip Tickets</h3>
            <p style="color: #64748b; font-size: 16px; margin: 0;">Refresh the page.</p>
          </td></tr>
        `;
        window.Modal?.show("Error loading trip tickets.", "error");
      });
  }
  // Generalized Stats Updater (for IT; extend for sections)
  ...
  // Placeholder for section stats (e.g., travel/trip; implement if HTML elements exist)
  function updateSectionStats(section, stats) {
    // Example: if (section === "travel") { update travel stat elements }
    console.log(`Updating ${section} stats:`, stats);
    // Extend with specific elements like document.getElementById(`stat-${section}-pending`)
  }
  ...
  // Ensure spinner CSS is injected
