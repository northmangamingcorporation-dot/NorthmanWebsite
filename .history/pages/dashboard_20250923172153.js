...
async function attachDashboard(user) {
  ...
  if (backBtn) {
    backBtn.addEventListener("click", async () => {
      ...
      if (window.Modal && window.Modal.confirm) {
        const confirmed = await window.Modal.confirm("Are you sure you want to logout?");
        if (!confirmed) return;
      } else {
...
// Add this in dashboard.js before the handler
function waitForNGC(callback, attempts = 20) {
  if (window.mount && window.renderHome) {
    callback();  // Ready!
    return;
  }
  if (attempts > 0) {
    setTimeout(() => waitForNGC(callback, attempts - 1), 100);
  } else {
    console.error("NGC functions timed out—check script loading.");
  }
}

// Use it to attach handler only when ready
waitForNGC(() => {
  // Your backBtn handler code here
});
