...
async function viewAccomplishment(id) {
  ...
  try {
    ...
    if (photos.length > 0) {
      ...
      if (photos.length > 10) {
        contentHTML += `<p style="text-align: center; color: #6b7280;">... and ${photos.length - 10} more photos</p>`;
      }
      contentHTML += '</div></div>';
    } else if (photoCount > 0) {
      contentHTML += '<p style="color: #f59e0b; text-align: center;">Photos were sent to Telegram but could not be retrieved at this time.</p>';
    } else {
