...
async function attachAdminDashboard(admin) {
    ...
    unsubscribe = ordersCol.onSnapshot(
      snapshot => {
        ...
        snapshot.forEach(doc => {
          ...
          // ✅ Row click → show request details
          row.addEventListener("click", (e) => {
            if (!e.target.classList.contains("btn")) { // ignore clicks on buttons
              showRequestDetails(order);
            }
          });
  ...
  function showRequestDetails(order) {
