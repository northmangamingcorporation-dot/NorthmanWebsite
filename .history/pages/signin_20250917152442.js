...
function attachSignIn() {
  document.getElementById("signUpBtn").addEventListener("click", async () => {
    ...
    try {
      ...
      console.log("OTP:", otp); // Replace with email sending in real case

      const userConfirmed = confirm(`Your OTP is ${otp}. Enter OK if correct.`); 
      // Simple browser confirm as demo; replace with actual email send + verification

      if (!userConfirmed) return showError("OTP verification failed");
      ...
      setTimeout(() => window.mountLogin(), 1500);
