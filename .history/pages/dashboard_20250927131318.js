...
// Assumes openAccomplishmentModal is defined (unified mount + attach); Added graceful error handling for modal open
async function loadAccomplishments(user) {
  ...
  try {
    ...
    snapshot.forEach(doc => {
      ...
      html += `
        <tr>
          <td data-label="Report ID">${doc.id}</td> <!-- Fixed: Use doc.id for Firestore ID -->
          ...
          <td data-label="Action">
            <button onclick="openAccomplishmentModal('${doc.id}')" 
                    style="background: none; border: none; color: #0ea5a4; cursor: pointer; padding: 5px 10px; border-radius: 4px; transition: background 0.2s;"
                    onmouseover="this.style.background='#e0f2fe'" 
                    onmouseout="this.style.background='none'"
                    aria-label="View accomplishment report ${doc.id}">View</button>
          </td>
        </tr>
      `;
    });
    ...
    tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; color: #ef4444; padding: 20px;">Error loading reports. Please try again.</td></tr>';
  }
}

// Unified function to open modal (combines mount and attach for easier use in onclick)
// This should be defined alongside the modal functions (mountAccomplishmentModal, etc.)
// Usage in onclick: openAccomplishmentModal('doc-id')
async function openAccomplishmentModal(id, optionalPreFetchedData = null) {
  try {
    const mountedData = await mountAccomplishmentModal(id, optionalPreFetchedData);
    if (mountedData) {
      attachAccomplishmentModal(mountedData);
    } else {
      console.error('Failed to mount data for modal');
      alert('Failed to load report details. Please try again.');
    }
  } catch (error) {
    console.error('Error opening accomplishment modal:', error);
    alert('Error opening report. Please check your connection and try again.');
  }
}
