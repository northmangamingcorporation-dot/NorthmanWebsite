            ...
            <div>
              ...
              <div>
                <span style="
                  ...
                  background: rgba(${rgbaValue}, 0.15);
                  color: ${statusColor};
                  border: 1px solid rgba(${rgbaValue}, 0.3);
...
function showRestDetailsModal(id, rest) {
  ...
  const statusColor = statusColors[rest.status] || '#f59e0b';
  const rgbaValue = statusRgba[rest.status] || '245, 158, 11';
...
// Add event listeners for tab buttons to switch tabs
document.addEventListener('DOMContentLoaded', () => {
  const tabButtons = document.querySelectorAll('.tab-btn');
  tabButtons.forEach(button => {
    button.addEventListener('click', () => {
      const targetTab = button.getAttribute('data-tab');
      switchTab(`${targetTab}-content`);
