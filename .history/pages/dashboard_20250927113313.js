...
async function loadAccomplishments(user) {
  ...
  try {
    ...
    snapshot.forEach(doc => {
      ...
      html += `
        <tr>
          <td data-label="Report ID">${data.id}</td>
          <td data-label="Description">${data.descriptionOfService?.substring(0, 50) || 'N/A'}...</td>
          <td data-label="Date">${date}</td>
          <td data-label="Status"><span style="color: #10b981; font-weight: 600;">${data.status || 'Submitted'}</span></td>
          <td data-label="Photos">${photoIndicator}</td>
          <td data-label="Action">
            <button onclick="viewAccomplishment('${doc.id}')" style="background: none; border: none; color: #0ea5a4; cursor: pointer;">View</button>
          </td>
        </tr>
      `;
    });
...
// UPDATED: View detailed accomplishment (compatible with enhanced Modal - raw HTML support)
async function viewAccomplishment(id) {
  if (!id || !window.db) {
    console.error('Invalid ID or database not available');
    if (window.Modal && window.Modal.show) {
      window.Modal.show('Error: Invalid report ID or database not available.', 'error');
    } else {
  ...
  try {
    ...
    const data = doc.data();
    console.log('Fetched data:', data);  // Debug: Check if data loads

    // Step 2: Fetch Telegram photos if available (non-blocking)
    let photos = [];
    const photoCount = data.telegramPhotos ? data.telegramPhotos.length : 0;
    if (photoCount > 0 && window.TelegramConnect && typeof window.TelegramConnect.getDataById === 'function') {
      try {
        photos = await window.TelegramConnect.getDataById(id);
        console.log(`Retrieved ${photos.length} photos for ID: ${id}`, photos);  // Debug: Check photos
      } catch (photoErr) {
    ...
    // Step 3: Build modal content (details + gallery) - no custom close button
    let modalContent = `
    ...
    if (photos.length > 0) {
      ...
      `;
      photos.slice(0, 10).forEach((photo, index) => {  // Limit to 10 for modal size
        ...
        modalContent += `
          <div class="photo-item" style="margin: 10px; text-align: center; max-width: 200px;">
            ${thumbSrc}
      ...
      if (photos.length > 10) {
        modalContent += `<p style="text-align: center; color: #6b7280;">... and ${photos.length - 10} more photos</p>`;
      }
      modalContent += '</div></div>';
    } else if (photoCount > 0) {
      modalContent += '<p style="color: #f59e0b; text-align: center;">Photos were sent to Telegram but could not be retrieved at this time.</p>';
    } else {
      modalContent += '<p style="color: #6b7280; text-align: center; font-style: italic;">No photos attached to this report.</p>';
    }

    modalContent += '</div>';  // Close outer div (no custom close button)

    console.log('Modal content built, attempting to show...');  // Debug

    // Step 4: Show modal (use rawHtml=true for complex content)
    if (window.Modal && window.Modal.show) {
      window.Modal.show(modalContent, 'info', 0, true, 'Accomplishment Report Details');  // rawHtml=true, customTitle
      console.log('Modal.show called successfully');  // Debug
    } else {
      console.warn('window.Modal not available - falling back to alert');
      // Enhanced fallback: Full details + photo URLs
      let fallbackText = `Accomplishment Report Details\n\nReport ID: ${data.id}\nDescription: ${data.descriptionOfService || 'N/A'}\nDate Submitted: ${data.dateSubmitted ? new Date(data.dateSubmitted.toDate()).toLocaleString() : 'N/A'}\nStatus: ${data.status || 'Submitted'}\n\n`;
      Object.entries(data).filter(([key]) => !['id', 'uniquekey', 'status', 'dateSubmitted', 'updatedAt', 'telegramPhotos'].includes(key)).forEach(([key, value]) => {
        const val = (value && value.toDate) ? value.toDate().toLocaleString() : value;
        fallbackText += `${key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}: ${val || 'N/A'}\n`;
      });
      fallbackText += `\nPhotos: ${photos.length}`;
      if (photos.length > 0) {
        fallbackText += `\n\nPhoto URLs (copy for viewing):\n`;
        photos.forEach((photo, i) => fallbackText += `${i+1}. ${photo.url}\n`);
      } else if (photoCount > 0) {
        fallbackText += '\nPhotos sent to Telegram but unavailable.';
      } else {
        fallbackText += '\nNo photos attached.';
      }
      alert(fallbackText);
    ...
    if (window.Modal && window.Modal.show) {
      window.Modal.show(errorMsg, 'error');
    } else {
