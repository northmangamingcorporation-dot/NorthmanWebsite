...
// --- Enhanced Attach Dashboard with HR Features ---
async function attachDashboard(user) {
  try {
    // Initialize tabs first
    initializeTabs();
    
    // Attach accomplishments section
    ...
    // Leave Request Button - NEW
    const leaveRequestBtn = document.getElementById("leaveRequestBtn");
    if (leaveRequestBtn) {
      leaveRequestBtn.addEventListener("click", showLeaveRequestModal);
    }

    // Early Rest Day Button - NEW
    const earlyRestBtn = document.getElementById("earlyRestBtn");
    if (earlyRestBtn) {
      earlyRestBtn.addEventListener("click", showEarlyRestModal);
    }
    ...
    // Update stats periodically - NEW
    updateDashboardStats(user);
    window.dashboardRefreshInterval = setInterval(() => updateDashboardStats(user), 30000);
