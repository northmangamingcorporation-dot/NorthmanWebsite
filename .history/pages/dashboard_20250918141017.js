...
async function attachDashboard(user) {
  ...
  let unsubscribe = null; // snapshot listener cleanup

  // Load requests for this user
  function loadRequests() {
    // Clean up old snapshot listener if any
  ...
  function statusColor(status) {
    switch (status) {
      ...
      default: return "orange"; // pending or unknown
    }
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    btn.addEventListener("click", () => {
      closeAllModals?.(); // optional, if you have that
      mountRequestModal(); // ✅ creates + shows modal from request_modal.js
    });
  });
