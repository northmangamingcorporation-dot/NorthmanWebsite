...
function loadLeaveRequests() {
  ...
  if (!window.db) {
    showEmptyState(leaveTable);
    return;
  }

  const currentUser = window.currentUser?.username || '';
  
  window.db.collection("leave_requests")
    ...
    .onSnapshot((snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        html += renderLeaveRow(doc.id, type, startDate, endDate, days, status);
      });
      ...
      leaveTable.innerHTML = `
        <tr>
          <td colspan="6" style="padding: 40px; text-align: center;">
            ...
            <p style="margin-top: 16px; color: #64748b;">Error loading leave requests</p>
          </td>
        </tr>
      `;
    });
...
function getLeaveIcon(type) {
  const icons = {
    ...
    bereavement: 'hand-holding-heart',
    personal: 'user',
    other: 'calendar-day'
  };
...
function renderLeaveRow(id, type, startDate, endDate, days, status) {
  const statusColors = {
  ...
  return `
    <tr style="transition: all 0.3s ease;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'">
      <td style="padding: 16px; border-bottom: 1px solid #f1f5f9;">
        <div style="display: flex; align-items: center; gap: 12px;">
          <i class="fas fa-${getLeaveIcon(type)}" style="color: #22c55e; font-size: 18px;"></i>
          <span style="font-weight: 600; color: #0f172a; text-transform: capitalize;">${leaveTypeLabels[type] || type}</span>
        </div>
      </td>
      <td style="padding: 16px; border-bottom: 1px solid #f1f5f9; color: #475569; font-weight: 500;">${startDate}</td>
      <td style="padding: 16px; border-bottom: 1px solid #f1f5f9; color: #475569; font-weight: 500;">${endDate}</td>
      <td style="padding: 16px; border-bottom: 1px solid #f1f5f9; color: #0f172a; font-weight: 700; font-size: 16px;">${days}</td>
      <td style="padding: 16px; border-bottom: 1px solid #f1f5f9;">
        <span style="
      ...
      <td style="padding: 16px; border-bottom: 1px solid #f1f5f9;">
        <button onclick="viewLeaveDetails('${id}')" style="
          ...
          align-items: center;
          gap: 8px;
        " onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 16px rgba(34, 197, 94, 0.3)'"
      ...
      ">
        ...
        ">
          ...
          ${user?.department === "IT" ? `<div class="action-card" onclick="window.showTicketInputModal()" style="
            ...
            align-items: center;
            gap: 16px;
          " onmouseover="this.style.borderColor='#ef4444'; this.style.transform='translateY(-4px)'; this.style.boxShadow='0 12px 24px rgba(239, 68, 68, 0.15)';"
            onmouseout="this.style.borderColor='#e2e8f0'; this.style.transform='translateY(0)'; this.style.boxShadow='none';">
            <div style="
              width: 60px;
              height: 60px;
...
// Enhanced Render Ticket Error Report Section with Filtering and Total
    ...
    ">
      ...
      ">
        <div>
          ...
          <p style="margin: 0; color: #64748b; font-size: 14px;">
            <i class="fas fa-calendar-day" style="color: #10b981; margin-right: 6px;"></i>
            <span id="reportDateTitle">Today's Report</span> • Paste ticket dumps, analyze automatically, and track booth errors
          </p>
        </div>
      ...
      ">
        ...
        ">
          ...
          ">
            <tr>
              <th style="text-align: left; padding: 18px 16px; color: #475569; font-weight: 700; font-size: 13px; text-transform: uppercase;">
                <i class="fas fa-ticket-alt" style="margin-right: 6px;"></i>Ticket Code
              </th>
              <th style="text-align: left; padding: 18px 16px; color: #475569; font-weight: 700; font-size: 13px; text-transform: uppercase;">
                <i class="fas fa-user" style="margin-right: 6px;"></i>Teller
              </th>
              <th style="text-align: left; padding: 18px 16px; color: #475569; font-weight: 700; font-size: 13px; text-transform: uppercase;">
                <i class="fas fa-tag" style="margin-right: 6px;"></i>Type
              </th>
              <th style="text-align: left; padding: 18px 16px; color: #475569; font-weight: 700; font-size: 13px; text-transform: uppercase;">
                <i class="fas fa-user-edit" style="margin-right: 6px;"></i>Submitted By
              </th>
              <th style="text-align: left; padding: 18px 16px; color: #475569; font-weight: 700; font-size: 13px; text-transform: uppercase;">
                <i class="fas fa-clock" style="margin-right: 6px;"></i>Date
              </th>
            </tr>
          </thead>
              ...
              <td colspan="5" style="padding: 60px 20px; text-align: center; border: none;">
                <div style="display: flex; flex-direction: column; align-items: center; gap: 16px;">
                  ...
                  <div>
                    ...
                    <p style="margin: 0; color: #94a3b8; font-size: 14px;">
                      Click "Report Teller" to paste and analyze a ticket
                    </p>
                  </div>
                </div>
              </td>
          ...
          <tfoot style="background: linear-gradient(135deg, #f9fafb, #f1f5f9); border-top: 2px solid #e2e8f0;">
            <tr>
              <td colspan="5" style="padding: 16px; text-align: right;">
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
                  <div style="color: #64748b; font-size: 14px; font-weight: 500;">
                    <i class="fas fa-info-circle" style="margin-right: 6px;"></i>
                    <span id="ticketsFilterInfo">Showing all tickets</span>
                  </div>
                  <div style="display: flex; align-items: center; gap: 16px;">
                    <span style="color: #334155; font-size: 15px; font-weight: 700;">
                      <i class="fas fa-list-ol" style="margin-right: 8px; color: #10b981;"></i>
                      Total: <span id="ticketsTotalCount" style="color: #10b981; font-size: 18px;">0</span>
                    </span>
                  </div>
                </div>
              </td>
...
function filterTickets() {
  ...
  for (let row of rows) {
    ...
    const ticketCode = cells[0].textContent.toLowerCase();
    const teller = cells[1].textContent.toLowerCase();
    const type = cells[2].textContent.toLowerCase();
    const submitter = cells[3].textContent.toLowerCase();
    
    const matchesSearch = searchValue === '' || 
                         ticketCode.includes(searchValue) || 
                         teller.includes(searchValue) || 
  ...
  updateTicketTotal(visibleCount);
  updateFilterInfo(searchValue, typeFilter);
}

// Sort tickets by selected criteria
function sortTicketsBy() {
...
function clearTicketFilters() {
  ...
  document.getElementById('ticketSortBy').value = 'date-desc';
  filterTickets();
}

// Update total count display
function updateTicketTotal(count) {
  const totalElement = document.getElementById('ticketsTotalCount');
  if (totalElement) {
...
function updateFilterInfo(searchValue, typeFilter) {
  const infoElement = document.getElementById('ticketsFilterInfo');
  if (!infoElement) return;
  
  if (searchValue === '' && typeFilter === '') {
    infoElement.textContent = 'Showing all tickets';
  } else {
    let filters = [];
    if (searchValue) filters.push(`search: "${searchValue}"`);
    if (typeFilter) filters.push(`type: ${typeFilter}`);
    infoElement.textContent = `Filtered by ${filters.join(', ')}`;
  }
}

// Update the date title
function updateReportDateTitle() {
  const titleElement = document.getElementById('reportDateTitle');
  if (titleElement) {
    const today = new Date();
    const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
    titleElement.textContent = today.toLocaleDateString('en-US', options);
  }
}

// Call this when the page loads
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', updateReportDateTitle);
} else {
  updateReportDateTitle();
}
