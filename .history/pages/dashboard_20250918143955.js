...
async function attachDashboard(user) {
  ...
  if (newRequestBtn) {
    newRequestBtn.addEventListener("click", () => {
      closeAllModals?.();
      mountRequestModal(); // ✅ modal now works
    });
  }
...
function mountDashboard(user) {
  ...
  attachDashboard(user); // Firestore table logic
}

// Helper: log all buttons
function logAllButtons() {
  ...
  buttons.forEach(btn => {
    ...
    const visible = !!(btn.offsetParent); // true if rendered and not hidden
    console.log(`Button: id="${id}", class="${classes}", visible=${visible}`);
  });
}

// Call after dashboard is mounted
setTimeout(() => {
  logAllButtons();
}, 100);
...
window.logAllButtons = logAllButtons;
