...
// NEW: View detailed accomplishment (fetches data + Telegram photos)
async function viewAccomplishment(id) {
  if (!id || !window.db) {
    console.error('Invalid ID or database not available');
    if (window.Modal && window.Modal.show) {
      window.Modal.show('Error: Invalid report ID.', 'error');
    } else {
  ...
  try {
    ...
    if (photoCount > 0 && window.TelegramConnect && typeof window.TelegramConnect.getDataById === 'function') {
      try {
        ...
        console.log(`Retrieved ${photos.length} photos for ID: ${id}`);
      } catch (photoErr) {
    ...
    // Step 3: Build modal content (details + gallery)
    let modalContent = `
    ...
    modalContent += `
        <div style="text-align: center; margin-top: 20px; padding-top: 15px; border-top: 1px solid #e5e7eb;">
          <button onclick="this.closest('.modal-content').parentElement.remove()" style="background: #ef4444; color: white; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer;">Close</button>
        </div>
      </div>
    `;
    ...
    // Step 4: Show modal
    if (window.Modal && window.Modal.show) {
      window.Modal.show(modalContent, 'info');  // Or 'success'/'warning' as needed
    } else {
      // Fallback: Alert for details (no photos)
      const detailsText = `Report ID: ${data.id}\nDescription: ${data.descriptionOfService || 'N/A'}\nDate: ${data.dateSubmitted ? new Date(data.dateSubmitted.toDate()).toLocaleString() : 'N/A'}\nStatus: ${data.status || 'Submitted'}\nPhotos: ${photos.length}`;
      alert(detailsText);
      if (photos.length > 0) {
        console.log('Photos (view in console):', photos);  // For debugging
      }
    }
