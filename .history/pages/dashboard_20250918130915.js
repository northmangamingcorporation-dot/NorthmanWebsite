...
function renderDashboard(user = { username: "Employee" }) {
  return `
    ...
    <div class="container app">
      <aside class="sidebar">
        <h3>Northman</h3>
        <a href="#" data-route="requests">📑 Requests</a>
        <a href="#" data-route="travel">✈️ Travel Orders</a>
        <a href="#" data-route="driver">🚐 Driver Trip Tickets</a>
        <a href="#" data-route="it">💻 IT Service Orders</a>
        <a href="#" data-route="settings">⚙️ Settings</a>
        <button class="btn" id="backBtn" style="margin-top:14px;">Logout</button>
      </aside>
...
async function attachDashboard(user) {
  ...
  document.getElementById("newRequestBtn").addEventListener("click", async () => {
    const type = await Modal.confirm("Create Travel Order (OK), Driver Trip Ticket (Cancel)?");
    const reqType = type ? "Travel Order" : "Driver Trip Ticket";
    const details = prompt("Enter request details:");
