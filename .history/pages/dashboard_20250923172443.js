  ...
  if (newRequestBtn) {
    newRequestBtn.addEventListener("click", () => {
      if (window.mountRequestModal) {
        ...
        console.warn("mountRequestModal not defined. Please implement request modal.");
        if (window.Modal && window.Modal.show) {
...
// Enhanced Back Button Handler in dashboard.js
// (Integrates NGC home page redirect with safety checks and UX polish)
  ...
  if (backBtn) {
    backBtn.addEventListener("click", async (e) => {
      ...
      try {
        // Enhanced logout confirmation with fallback
        let confirmed = true;
        if (window.Modal && typeof window.Modal.confirm === "function") {
          confirmed = await window.Modal.confirm("Are you sure you want to logout? All unsaved changes will be lost.", {
            confirmText: "Logout",
            cancelText: "Stay",
            type: "warning"
          });
        } else if (confirm("Are you sure you want to logout? All unsaved changes will be lost.")) {
          confirmed = true;
        } else {
          confirmed = false;
        }
        ...
        if (!confirmed) {
          // Optional: Provide feedback on cancel
          if (window.Modal && typeof window.Modal.show === "function") {
            window.Modal.show("Logout cancelled.", "info", 2000);
          }
          return;
        }
        ...
        // Safely unsubscribe if defined (e.g., from Firebase realtime listeners)
        ...
        // Animate logout button state (save original for reset)
        const originalHTML = backBtn.innerHTML;
        const originalDisabled = backBtn.disabled;
        ...
        if (window.Modal && typeof window.Modal.show === "function") {
          window.Modal.show("Logging out...", "loading");
        }
        
        // Delay for animation and cleanup
        setTimeout(() => {
          try {
            ...
            // Redirect to NGC home page using index.js functions
            if (window.mount && window.renderHome) {
              window.mount(window.renderHome());  // This renders the full NGC home
              console.log("Redirected to NGC home page.");
              
              // Optional: Re-attach NGC actions and styles if needed (usually handled in mount)
              if (window.attachActions) window.attachActions();
              if (window.injectMainPageStyles) window.injectMainPageStyles();
            } else if (window.mountLogin) {
              // Fallback to login if NGC not available
              window.mountLogin();
              console.log("Fallback: Redirected to login page.");
            } else {
              // Ultimate fallback: Reload or redirect to home
              window.location.href = window.location.origin + "/index.html";  // Adjust path as needed
              console.log("Fallback redirect initiated.");
            }
            
            // Reset button state (though page will change)
            backBtn.innerHTML = originalHTML;
            backBtn.disabled = originalDisabled;
            backBtn.removeAttribute("aria-busy");
            
            // Success feedback
            if (window.Modal && typeof window.Modal.show === "function") {
              window.Modal.show("Logout successful. Welcome back to NGC!", "success", 3000);
            }
            
          } catch (redirectError) {
            console.error("Error during redirect:", redirectError);
            if (window.Modal && typeof window.Modal.show === "function") {
              window.Modal.show("Logout completed, but redirect failed. Please refresh the page.", "warning");
            } else {
              alert("Logout completed. Please refresh the page.");
            }
        ...
        if (backBtn) {
          backBtn.innerHTML = '<i class="fas fa-sign-out-alt"></i> Logout';  // Assuming original icon/text
        ...
        if (window.Modal && typeof window.Modal.show === "function") {
          window.Modal.show(`Logout failed: ${error.message}. Please try again or contact support.`, "error");
        } else {
    ...
    console.log("Dashboard back/logout button handler attached successfully.");
  } else {
    console.warn("Back button (#backBtn) not found in DOM. Skipping handler attachment.");
  }
