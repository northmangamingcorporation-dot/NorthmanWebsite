...
function renderDashboard(user = { username: "Employee" }) {
  return `
    ...
    <!-- Modal -->
    <div id="requestTypeModal" class="modal hidden" 
      style="position:fixed; inset:0; background:rgba(0,0,0,0.5); display:flex; align-items:center; justify-content:center;">
      <div class="modal-content" style="background:#fff; padding:20px; border-radius:12px; max-width:400px; width:100%;">
        ...
        <div style="text-align:right; margin-top:16px;">
          <button onclick="RequestTypeModal.hide()" class="btn">Close</button>
        </div>
      </div>
    </div>
  `;
...
async function attachDashboard(user) {
  ...
  // New Request button -> show modal
  document.getElementById("newRequestBtn").addEventListener("click", () => {
    RequestTypeModal.show();
  });

  // Hook up request type buttons inside modal
