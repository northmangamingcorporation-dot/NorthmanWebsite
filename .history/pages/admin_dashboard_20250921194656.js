...
function renderAdminDashboard(admin = { username: "Admin" }) {
  return `
    ${renderNav()}
      ...
      <main class="main">
        <div class="header">
          <div>
            ...
            <div style="color:var(--muted)">Welcome, ${admin.username}</div>
          </div>
          <div style="display:flex; gap:12px; align-items:center">
            <img src="https://i.pravatar.cc/44?u=admin" alt="avatar" 
...
async function attachAdminDashboard(admin) {
  ...
  function loadOrders() {
    ...
    unsubscribe = ordersCol
      //.orderBy("dateSubmitted", "desc") // optional index
      .onSnapshot(
        snapshot => {
...
function mountAdminDashboard(prefi) {
