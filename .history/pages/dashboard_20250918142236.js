...
function mountDashboard(user) {
  ...
  // Attach events after DOM elements exist
  const newReqBtn = document.getElementById("newRequestBtn");
  if (newReqBtn) {
    newReqBtn.addEventListener("click", () => {
      closeAllModals?.();
      mountRequestModal(); // ✅ modal now shows
    });
  }

  const backBtn = document.getElementById("backBtn");
  if (backBtn) {
    backBtn.addEventListener("click", () => window.mountLogin());
  }

  attachDashboard(user); // Firestore listener etc.
}
