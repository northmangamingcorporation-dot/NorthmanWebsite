...
async function getStaffTasks() {
  ...
  try {
    ...
    const clientsSnapshot = await db.collection("clients").where("assignedStaff", "==", staff.username).get();
    const staffList = clientsSnapshot.docs.map(doc => doc.data());

    // 2. For each staff, count tasks in it_service_orders
    for (const staff of staffList) {
      const querySnapshot = await db.collection("it_service_orders")
        .where("assignedStaff", "==", staff.username)
        .get();
