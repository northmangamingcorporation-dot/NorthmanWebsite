...
// --- Dynamically Populate Department Filter from Table Column ---
function populateDepartmentFilter() {
  ...
  const uniqueDepts = new Set(); // Use Set for case-insensitive deduplication

  // Scan rows for unique departments (skip header/loading/empty rows)
  for (let i = 1; i < rows.length; i++) { // Skip header (index 0)
    const row = rows[i];
    const cells = row.getElementsByTagName("td");
    
    if (cells.length < 3 || row.classList.contains("loading-state")) {
      continue; // Skip invalid or loading rows
    }
    ...
    if (deptText && deptText !== 'N/A' && deptText !== '') {
      // Add to Set (lowercase for uniqueness, but store original casing)
      uniqueDepts.add(deptText.toLowerCase() + '|' + deptText); // Hack to preserve casing
    }
  }
  ...
  // Convert Set to array, extract original casing, sort alphabetically
  const sortedDepts = Array.from(uniqueDepts)
    .map(item => item.split('|')[1]) // Get original casing
    .sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' })); // Case-insensitive sort

  // Add sorted options
  sortedDepts.forEach(dept => {
    const option = document.createElement("option");
    option.value = dept.toLowerCase(); // Value in lowercase for filtering consistency
    option.textContent = dept; // Display original casing
    filterSelect.appendChild(option);
  });

  console.log(`Department filter populated with ${sortedDepts.length} unique departments:`, sortedDepts); // Debug log (remove if unwanted)
}

// --- Example: Call After Table Population (Integrate with Your Load Function) ---
/*
async function loadClients() {
  // Your existing code: Query Firebase, populate #adminClientsTable
  ...
  snapshot.forEach(doc => {
    ...
    tbody.innerHTML += `
      <tr>
        ...
        <td>${data.department || 'N/A'}</td> <!-- Column 2: Department -->
  ...
  // Dynamically populate filter AFTER data is added
  populateDepartmentFilter();

  // Re-attach filters (if using your initializeSearchAndFilters)
