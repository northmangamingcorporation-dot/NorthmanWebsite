...
async function attachITAdminDashboard(admin) {
  ...
  // Enhanced load orders with real-time update
function loadOrders() {
  ...
  row.innerHTML = `
    ...
    <td>
      <div class="action-buttons">
        ...
        <button class="btn-icon edit-btn" data-id="${docId}" title="Edit">
          <i class="fas fa-edit"></i>
        </button>
      </div>
    </td>
...
// --- Initialize Search and Filters (updated with clear button handler for requests) ---
function initializeSearchAndFilters() {
  ...
  if (requestSearch) {
    const debouncedRequestsFilter = debounce(function(searchTerm) {
      console.log("Requests search event fired:", searchTerm); // Debug (remove later)
      filterTable("adminRequestsTable", searchTerm, [1, 2, 4]); // Employee, Type, Details
    }, 300);

    requestSearch.addEventListener("input", function() {
      debouncedRequestsFilter(this.value);
      updateClearButtonVisibility(this.value); // Show/hide clear button
    });

    // Clear button handler (#clearSearchBtn)
    const clearSearchBtn = document.getElementById("clearSearchBtn");
    if (clearSearchBtn) {
      clearSearchBtn.addEventListener("click", function(e) {
        e.preventDefault();
        requestSearch.value = "";
        debouncedRequestsFilter(""); // Reset search (show all)
        updateClearButtonVisibility(""); // Hide button
        requestSearch.focus();
      });
    }
    ...
    // Initial visibility
    updateClearButtonVisibility(requestSearch.value);
  }

  // Clients search (no clear button in HTML)
  const clientSearch = document.getElementById("searchClientsInput");
  if (clientSearch) {
    const debouncedClientsFilter = debounce(function(searchTerm) {
      filterTable("adminClientsTable", searchTerm, [0, 1, 2, 4]); // Name, Username, Dept, Email
    }, 300);

    clientSearch.addEventListener("input", function() {
      debouncedClientsFilter(this.value);
    });
  }
  ...
  if (statusFilter) {
    statusFilter.addEventListener("change", function() {
      filterTableByColumn("adminRequestsTable", 6, this.value);
    });
  }
  ...
  if (typeFilter) {
    typeFilter.addEventListener("change", function() {
      filterTableByColumn("adminRequestsTable", 2, this.value);
    });
  }

  // Department filter (clients)
  const deptFilter = document.getElementById("departmentFilter");
  if (deptFilter) {
    deptFilter.addEventListener("change", function() {
      filterTableByColumn("adminClientsTable", 2, this.value);
    });
  }
...
// --- Helper: Update Clear Button Visibility (for requests) ---
...
// --- Column Filter (exact match for dropdowns) ---
function filterTableByColumn(tableId, columnIndex, filterValue) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (cells.length === 0 || cells.length <= columnIndex) continue;
    
    const cell = cells[columnIndex];
    const cellText = (cell.textContent || cell.innerText || '').trim().toLowerCase();
    const lowerFilter = filterValue ? filterValue.toLowerCase() : '';
    
    let match = false;
    if (!lowerFilter) {
      match = true; // Show all
    } else if (cellText === lowerFilter) {
      match = true; // Exact match
    } else if (cellText.includes(lowerFilter)) {
      match = true; // Partial fallback
    }
...
// --- Load IT Requests (Example: Adapt to Your Firebase Query) ---
async function loadRequests() {
  try {
    const tbody = document.getElementById("adminRequestsTable");
    if (!tbody) return;

    // Show loading (matches your HTML colspan=9)
    tbody.innerHTML = `
      <tr>
        <td colspan="9" class="loading-state">
          <div class="loading-content">
    ...
    // Firebase query (adapt: e.g., db.collection("it_requests").orderBy("date", "desc"))
    const snapshot = await db.collection("it_requests").get();
    ...
    snapshot.forEach(doc => {
      ...
      // Row matches your headers: Checkbox, Employee, Type, Priority, Details, Date, Status, Assigned, Actions
      tbody.innerHTML += `
        <tr>
          <td><input type="checkbox" class="request-checkbox" value="${doc.id}"></td>
          <td>${data.employee || 'N/A'}</td> <!-- Col 1 -->
          <td>${data.type || 'N/A'}</td> <!-- Col 2: Type for filter -->
          <td><span class="priority-badge ${priority}">${priority.charAt(0).toUpperCase() + priority.slice(1)}</span></td>
          <td>${data.details || 'N/A'}</td> <!-- Col 4 -->
          <td>${date}</td> <!-- Col 5 -->
          <td><span class="status-badge ${status}">${status.charAt(0).toUpperCase() + status.slice(1)}</span></td> <!-- Col 6: Status for filter -->
          <td>${data.assigned || 'Unassigned'}</td> <!-- Col 7 -->
          <td>
            <button class="btn btn-sm btn-info" onclick="viewRequest('${doc.id}')">View</button>
            <button class="btn btn-sm btn-warning" onclick="editRequest('${doc.id}')">Edit</button>
          </td>
        </tr>
      `;
    });

    // Update stats (matches your HTML)
    document.getElementById("pendingCount").textContent = pending;
    document.getElementById("urgentCount").textContent = urgent;

    // Populate dynamic filters AFTER data
    ...
    // Optional: Init checkboxes/bulk/pagination here if needed
    console.log(`Loaded ${snapshot.size} requests (${pending} pending, ${urgent} urgent).`);
  } catch (error) {
    console.error("Error loading requests:", error);
    const tbody = document.getElementById("adminRequestsTable");
    tbody.innerHTML = `<tr><td colspan="9" class="error-state">Error loading requests. Please try again.</td></tr>`;
  }
}

// --- Load Clients (Example: Adapt to Your Firebase Query) ---
async function loadClients() {
  try {
    const tbody = document.getElementById("adminClientsTable");
    if (!tbody) return;

    // Show loading (matches your HTML colspan=8)
    tbody.innerHTML = `
      <tr>
        <td colspan="8" class="loading-state">
          <div class="loading-content">
    ...
    // Firebase query (adapt: e.g., db.collection("clients").orderBy("name"))
    ...
    snapshot.forEach(doc => {
      ...
      const lastActive = data.lastActive ? data.lastActive.toDate().toLocaleDateString() : 'Never';

      if (status === 'active') active++;
      total++;

      // Row matches your headers: Name, Username, Department, Position, Email, Status, Last Active, Actions
      tbody.innerHTML += `
        <tr>
          <td>${data.name || 'N/A'}</td>
          <td>${data.username || 'N/A'}</td>
          <td>${data.department || 'N/A'}</td> <!-- Col 2: Dept for filter -->
          <td>${data.position || 'N/A'}</td>
          <td>${data.email || 'N/A'}</td>
          <td><span class="status-badge ${status}">${status.charAt(0).toUpperCase() + status.slice(1)}</span></td>
          <td>${lastActive}</td>
          <td>
            <button class="btn btn-sm btn-info" onclick="viewClient('${doc.id}')">View</button>
            <button class="btn btn-sm btn-warning" onclick="editClient('${doc.id}')">Edit</button>
          </td>
        </tr>
      `;
    });
    // ... (continuation of snapshot.forEach loop from previous response)
    snapshot.forEach(doc => {
      const data = doc.data();
      const status = data.status || 'inactive';
      const lastActive = data.lastActive ? data.lastActive.toDate().toLocaleDateString() : 'Never';
    ...
    // Update stats (matches your HTML: #totalClients, #activeClients)
    document.getElementById("totalClients").textContent = total;
    document.getElementById("activeClients").textContent = active;

    // Populate dynamic department filter AFTER data is added
    populateDepartmentFilter();

    // Initialize search/filters
    initializeSearchAndFilters();

    console.log(`Loaded ${total} clients (${active} active).`);
  } catch (error) {
    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="8" class="error-state">
          <span>Error loading clients. Please try again.</span>
        </td>
      </tr>
    `;
  }
...
// --- Auto-Load Tables on Page Ready (Integrate with Your Main Script) ---
document.addEventListener("DOMContentLoaded", function() {
  // Load requests first (or in parallel if preferred)
  loadRequests();
  
  // Load clients
  loadClients();
  
  // Optional: Other initializations (e.g., modals, auth checks)
  console.log("Dashboard initialized: Tables and filters loaded.");
});
