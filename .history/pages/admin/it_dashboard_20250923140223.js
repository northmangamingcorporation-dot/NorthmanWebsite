...
<div id="dashboardSection" class="section">
  ...
  <h4>Your Task Overview</h4>
  <div class="stat-card it-manager-card">
    <div class="stat-label">IT Manager Tasks</div>
    <div class="it-manager-tasks">
...
async function loadITManagerTasks() {
  try {
    const querySnapshot = await db.collection("ITdepartment_tasks")
      .where("Posito", "==", "IT Manager")
      .get();
...
.it-manager-card .it-manager-tasks {
  margin-top: 10px;
  font-size: 14px;
  color: #1f2937;
  max-height: 200px;      /* optional scroll if too many tasks */
  overflow-y: auto;
}

.it-manager-card .it-manager-tasks div.task-item {
  padding: 6px 8px;
  margin-bottom: 6px;
  background: #f3f4f6;
  border-radius: 6px;
...
.it-manager-card .it-manager-tasks div.task-item span.status {
  font-weight: 600;
  font-size: 12px;
