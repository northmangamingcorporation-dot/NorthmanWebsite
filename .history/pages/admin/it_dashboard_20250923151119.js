    ...
    querySnapshot.forEach(doc => {
      ...
      taskDiv.innerHTML = `
        <div class="task-header">
          <div class="task-info">
            <span class="task-name">${task.title || task.type || "Untitled Task"}</span>
            <span class="task-meta">${formatDate(task.dateCreated)}</span>
          </div>
        ...
        <div class="task-actions">
          <button class="btn-icon" onclick="viewTaskDetails('${doc.id}')" title="View Details">
            <i class="fas fa-eye"></i>
          </button>
...
async function attachITAdminDashboard(admin) {
  ...
  // Enhanced load orders with real-time updates
  function loadOrders() {
  ...
  Promise.all([
    ordersCol.orderBy("dateSubmitted", "desc").get(),
    tasksCol.get()
  ])
    .then(([ordersSnapshot, tasksSnapshot]) => {
      const tbody = document.getElementById("adminRequestsTable");
      tbody.innerHTML = "";
      const stats = { pending: 0, approved: 0, denied: 0, cancelled: 0 };

      if (ordersSnapshot.empty || tasksSnapshot.empty) {
        tbody.innerHTML = createEmptyState(9, "No matching IT requests yet", "fas fa-clipboard-list");
        updateAdminStats(stats);
        return;
      }

      // Create a lookup for tasks by relatedId + staff
      const taskMap = {};
      tasksSnapshot.forEach(taskDoc => {
        const task = taskDoc.data();
        const key = `${task.relatedId}_${task.staff}`;
        taskMap[key] = task;
      });

      // Filter orders that have matching task
      ordersSnapshot.forEach(orderDoc => {
        const order = orderDoc.data();
        const key = `${orderDoc.id}_${order.assignedTo}`;
        const matchedTask = taskMap[key];

        if (!matchedTask) return; // skip if no matching task
        ...
        const row = createEnhancedRequestRow(orderDoc.id, order, createdAt, statusKey, admin);
    ...
    .catch(err => {
      console.error("Error loading orders and tasks:", err);
...
function initializeSearchAndFilters() {
  ...
  if (requestSearch) {
    requestSearch.addEventListener("input", debounce(function() {
      filterTable("adminRequestsTable", this.value, [1, 2, 4]); // Search in employee, type, description columns
    }, 300));
  }
