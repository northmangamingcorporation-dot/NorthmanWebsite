...
// --- Dynamically Populate Status Filter from Requests Table (Column 6) ---
function populateStatusFilter() {
  ...
  const uniqueStatuses = new Set(); // Case-insensitive deduplication

  // Scan rows for unique statuses (skip header/loading/empty rows)
  for (let i = 1; i < rows.length; i++) {
    const row = rows[i];
    const cells = row.getElementsByTagName("td");
    
    if (cells.length < 7 || row.classList.contains("loading-state")) {
      continue;
    }

    const statusCell = cells[6]; // Status column (index 6)
    const statusText = (statusCell.textContent || statusCell.innerText || '').trim();
    
    if (statusText && statusText !== 'N/A' && statusText !== '') {
      uniqueStatuses.add(statusText.toLowerCase() + '|' + statusText); // Preserve casing
    }
  }

  // Clear and repopulate (keep "All Status")
  ...
  sortedStatuses.forEach(status => {
    ...
    option.value = status.toLowerCase(); // Lowercase for filtering
    option.textContent = status; // Original casing for display
...
// --- Dynamically Populate Type Filter from Requests Table (Column 2) ---
function populateTypeFilter() {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (cells.length < 3 || row.classList.contains("loading-state")) {
      continue;
    }

    const typeCell = cells[2]; // Type column (index 2)
  ...
  // Clear and repopulate (keep "All Types")
...
// --- Dynamically Populate Department Filter from Clients Table (Column 2) ---
function populateDepartmentFilter() {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const deptCell = cells[2]; // Department column (index 2)
  ...
  // Clear and repopulate (keep "All Departments")
...
// --- Initialize Search and Filters (unchanged) ---
function initializeSearchAndFilters() {
  ...
  if (requestSearch) {
    const debouncedRequestsFilter = debounce(function(searchTerm) {
      console.log("Search input event fired:", searchTerm); // Debug
  ...
  if (clientSearch) {
    const debouncedClientsFilter = debounce(function(searchTerm) {
      filterTable("adminClientsTable", searchTerm, [0, 1, 2, 4]);
    }, 300);
...
// --- Column Filter (updated for exact match on dropdowns) ---
function filterTableByColumn(tableId, columnIndex, filterValue) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (!lowerFilter) {
      ...
      match = true; // Exact match for dropdowns
    } else if (cellText.includes(lowerFilter)) {
      match = true; // Fallback to partial if exact fails
    }
