...
function formatDateForInput(date) {
      ...
      form.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          if (referenceCode && teller) {
            const duplicateSnapshot = await window.db.collection('it_report_human_error')
          ...
          // Save to Firestore
          await window.db.collection('it_report_human_error').add(ticketData);
  ...
  function viewITRequestDetails(id) {
    ...
    window.db.collection('it_report_human_error').doc(id).get().then((doc) => {
      if (doc.exists) {
