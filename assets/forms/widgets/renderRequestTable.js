...
// UPDATED: loadOrders(user) - Fixed duplication with debounced full re-render
async function loadOrders(user) {
  ...
  if (collections.length === 0) {
    ...
    collections = [
      ...
      { col: window.db.collection("travel_orders"), type: 'Travel', label: 'Travel Orders', icon: 'fas fa-map-pin', color: '#10b981' },
  ...
  // Function to render a section (with divider and header) - UNCHANGED
      ...
      row.innerHTML = `
        <td style="padding: 16px 12px; font-weight: 600; color: #0f172a !important;">
          <div style="display: flex; align-items: center; gap: 8px;">
            <i class="${sectionType === 'IT' ? 'fas fa-cog' : sectionType === 'Travel' ? 'fas fa-map-pin' : 'fas fa-car'}" style="color: ${color}; font-size: 16px;"></i>
  ...
  // Function to handle empty state for all sections - UNCHANGED
  ...
  // NEW: Debounced full re-render function (prevents duplication from multiple snapshots)
  let renderTimeout = null;
  const debouncedRender = () => {
    if (renderTimeout) clearTimeout(renderTimeout);
    renderTimeout = setTimeout(async () => {
      console.log('Debounced full re-render triggered');
      ...
      // Query and render each section (full sync)
      for (const coll of collections) {
        try {
          ...
          console.error(`Error querying ${coll.type}:`, err);
        }
      }
      ...
      // Attach cancel events after rendering (fresh buttons)
      ...
      cancelButtons.forEach(btn => {
        ...
        btn.replaceWith(btn.cloneNode(true));  // Clone to remove old events
        const newBtn = btn.closest('tr').querySelector('.btn-cancel');  // Get cloned btn

        newBtn.addEventListener("click", async (e) => {
          e.stopPropagation();
          const id = newBtn.dataset.id;
          const reqType = newBtn.dataset.type;
          if (!id || !reqType) {
          ...
          try {
            ...
            newBtn.innerHTML = '<i class="fas fa-spinner fa-spin" style="margin-right: 4px;"></i> Cancelling...';

            // Get correct collection based on type
            let targetCol;
            switch (reqType) {
              case 'IT': targetCol = window.db.collection("it_service_orders"); break;
              case 'Travel': targetCol = window.db.collection("travel_orders"); break;
              case 'Drivers': targetCol = window.db.collection("drivers_trip_tickets"); break;
            ...
            newBtn.innerHTML = '<i class="fas fa-times" style="margin-right: 4px; color: #ef4444;"></i>Cancel';
            if (window.Modal && window.Modal.show) {
      ...
      console.log('Debounced re-render completed');
    }, 150);  // 150ms delay - enough for multiple snapshots to queue
  };

  // Initial empty state
  renderEmptyState();

  // Set up real-time listeners for each (dynamic) collection - FIXED: Just trigger debounced render
    ...
    const unsub = col
      ...
      .onSnapshot(
        (snapshot) => {
          ...
          debouncedRender();  // Trigger debounced full re-render (no more per-listener logic)
        },
        (err) => {
          ...
          debouncedRender();  // Still re-render others on error
        }
      );
  ...
  console.log("DB-based multiple request listeners set up successfully (with debounce)");
}
