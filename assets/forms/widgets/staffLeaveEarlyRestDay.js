...
// Update createLeaveRequestRow in dashboard.js
...
async function handleLeaveApproval(docId, status, request) {
  ...
  try {
    ...
    // If department denies, immediately set final status to denied
    if (status === 'denied') {
      updateData.status = 'denied';
    }
    // If department approves, keep status as pending (waiting for HR)
    // Don't change the overall status yet

    await window.db.collection('leave_requests').doc(docId).update(updateData);

    showNotification(`Leave request ${status} by department`, 'success');
...
async function handleRestApproval(docId, status, request) {
  ...
  try {
    ...
    showNotification(`Rest day request ${status} by department`, 'success');
